...
- 전 과정: 7월 [PROCESS_202507_202607_KR.md](docs/pipeline/PROCESS_202507_202607_KR.md) ·
  ...
  [WORKLOG_20260825_26_KR.md](docs/worklog/WORKLOG_20260825_26_KR.md)

#### 8월 산출 결과 (`gee` 쪽, 2026-08-26 전량 완료)

| 대권역 | 궤도 | 제약 km² | 전 | 후 | 변화율 |
| --- | --- | ---: | ---: | ---: | ---: |
| 낙동강 | ASC54 | 1,007.56 | 335.08 | 313.90 | −6.3% |
| 섬진강 | ASC127 | 202.64 | 73.04 | 62.97 | −13.8% |
| 영산강 | ASC127 | 192.49 | 75.96 | 73.46 | −3.3% |
| **금강** | ASC127 | 467.60 | 172.49 | 168.13 | **−2.5%** |
| **한강** | ASC54 | 971.75 | 269.06 | 279.87 | **+4.0%** ⚠ |

⚠ **한강만 증가**다. 임계값 인공물인지 확인하려고 두 해 dB 분포를 재보니 뭍이
통째로 +0.36 dB 밝아졌고 **Edge-Otsu 임계도 정확히 같은 폭으로 따라가** 두 해
모두 뭍 중앙값보다 6.27 dB 아래라는 같은 자리였다 — 임계 이동이 면적을 부풀린
것은 아니다. 다만 **관측률 79%**(임진강·한강하구가 빠졌다)이고 **한강만 1A↔1D**라
계통 이동에 위성차와 토양수분이 섞여 분리되지 않는다. 가뭄 판정 근거 유역에서는
빼는 편이 안전하다. 상세: `2026_drought_map/REPORT_AUG_SAR_KR.md` §6-3.
...
- **빈 산출물은 성공처럼 보입니다.** 프레임이 external DEM 범위 밖이면 SNAP이
  오류 없이 정상 종료하고 배치는 "성공"으로 셉니다. 파일이 남으니 다음 실행은
  건너뛰어 **DEM을 넓혀도 영영 안 고쳐집니다.** 그래서 배치가 씬마다 유효화소를
  재고 **하한(기본 5%) 미달이면 실패로 세고 파일을 지웁니다**(`--min-valid`).
  전수 점검은 아래 도구로 합니다 ([ISSUES #24](docs/worklog/ISSUES_KR.md)).

  ```bash
  # 폴더 전수 — 유효화소·압축률을 표로. 씬당 30~90초
  conda run -n s1_snappy python -m s1.tools.audit.rtc_integrity

  # 재처리 대상 목록을 배치 --only 에 붙일 형태로
  conda run -n s1_snappy python -m s1.tools.audit.rtc_integrity --min-valid 20 --only-bad --ids
  ```

  ⚠ **유효화소가 낮다고 손상은 아닙니다.** 스와스가 기울어져 북쪽 정렬 사각형의
  네 모서리는 구조적으로 무효라, 북부 프레임은 기하만으로 **42%대**가 정상입니다
  (2026-07 12장 실측 42.9~66.8%). 지우는 선(5%)과 의심하는 선(40%)은 다릅니다.
...
- [PREPROCESSING_SPEC_KR.md](docs/pipeline/PREPROCESSING_SPEC_KR.md) — ⭐ **전처리 규격
  정본**: DEM 선택 근거(한반도는 `korea_peninsula_cop30.tif`), 그래프 8단계, 확정
  파라미터, 처리시간 실측(2-1절), POEORB→RESORB 대체(2-2절), 바꾸면 안 되는 것(7절)
- [BBOX_RANGES_KR.md](docs/pipeline/BBOX_RANGES_KR.md) — ⭐ **한반도 범위값 정본**:
  저장소에 흩어진 범위 9종을 용도별로 정리. **검색 bbox·DEM clip·판정 경계는 서로
  다른 물건**이고 섞으면 조용히 틀린다. 남한 전용(위도 39.x)과 한반도(43.x) 구분
- [GIT_WORKFLOW_KR.md](docs/GIT_WORKFLOW_KR.md) — **Git 작업 방침**: 기본은 main 직접
  커밋, 실험 브랜치는 며칠 안에 머지(이름이 내용을 설명 못 하면 머지 신호),
  **PR 기준은 "이미 만든 산출물의 의미가 바뀌는가"**(사양서 7절을 건드리면 PR)
...
- [WORKLOG_20260825_26_KR.md](docs/worklog/WORKLOG_20260825_26_KR.md) — **최신**:
  배치 유효화소 하한 검사(빈 산출물이 "성공"으로 집계되던 것), 유효화소 42%가
  정상인 이유, 한강·금강 8월 산출과 한강 증가 판정, 8월 쌍 파일이 두 유역을
  조용히 건너뛰게 하던 함정
- [WORKLOG_20260818_KR.md](docs/worklog/WORKLOG_20260818_KR.md) — 8월 남한 →
