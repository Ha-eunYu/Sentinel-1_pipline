...
### 1. `build_rtc_mosaic.py` — VRT 생성 시 NoData 미지정 ([build_rtc_mosaic.py:62](../../s1/tools/mosaic/build_rtc_mosaic.py#L62))
...
### 2. `download_hand.py` — 다운로드 무결성 검증 없음 ([download_hand.py:83-91](../../s1/tools/download/download_hand.py#L83-L91))
...
### 3. `build_baseline_water.py` — CRS 가정이 코드로 강제되지 않음 ([build_baseline_water.py:108](../../s1/tools/water/build_baseline_water.py#L108))
...
- **[download_s1.py:118](../../s1/stac/download_s1.py#L118)** `out_path.exists()` 스킵 검사가
  Range 헤더 설정·"Downloading" 출력 **이후**에 있습니다. 함수 맨 앞으로 이동.
- **[download_s1.py:205-217](../../s1/stac/download_s1.py#L205-L217)** 재시도 루프가 401/403/404
...
- `SNAP_BIN = r"C:\Program Files\snap\bin"` ([prepro_gpt.py:41](../../s1/preprocess/prepro_gpt.py#L41)),
...
- GRD 체인의 Speckle-Filter에 `numLooksStr` 미지정([prepro_grd_gpt.py:160](../../s1/preprocess/prepro_grd_gpt.py#L160)) —
...
- `import geopandas as gpd`가 두 번([check_kml_dam_korea.py:14,18](../../s1/tools/audit/check_kml_dam_korea.py#L14)).
...
이번 세션 신규/변경분: `build_grd_gtc_graph`+`--gtc`([prepro_grd_gpt.py](../../s1/preprocess/prepro_grd_gpt.py)),
[batch_grd_gtc.py](../../s1/tools/preprocess/batch_grd_gtc.py), geojson/yml 폴더 이동에 따른 경로 참조 갱신
...
- **파일**: [batch_grd_rtc.py](../../s1/tools/preprocess/batch_grd_rtc.py):46, [batch_grd_gtc.py](../../s1/tools/preprocess/batch_grd_gtc.py):51.
...
- **조치**: [RTC_VS_GTC_KR.md](../pipeline/RTC_VS_GTC_KR.md) 5절에 "GTC는 육안 비교
...
- **파괴적 작업 전 검증**: 씬 삭제·이동 전에 footprint 폴리곤 교차·픽셀
  좌표를 실제로 확인하고([SCENE_FOOTPRINT_REAUDIT_KR.md](../pipeline/SCENE_FOOTPRINT_REAUDIT_KR.md)),
