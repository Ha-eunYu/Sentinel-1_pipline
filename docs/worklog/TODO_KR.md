...
- [ ] 🔄 **`D635` + `B5F3` 단독 전처리 진행 중** (08-19 08:13 시작, 1프로세스,
      `-q 8 -c 14G`, peninsula DEM). 로그
      `temp/logs/_rtc_202608_solo_d635_b5f3.log`.
      - `D635` — 08-18 배치 큐에서 누락돼 있던 씬. 08-13 rel 134에서 **남한
        커버 최대(85.6%)** 프레임이라 빠지면 그 패스가 반쪽이 된다.
      - `B5F3` — 한반도 교집합 **0.00%**라 1% 규칙으로 제외했던 씬(사용자
        지시로 처리). ⚠ **산출물은 분석에 쓰지 말 것** — 육지가 없다.
        선별 기준([사양서 4절](../pipeline/PREPROCESSING_SPEC_KR.md))은 그대로다.
      - 이 실행이 **단독 vs 2병렬 비교의 첫 측정치**다(아래 항목).
        ⚠ **비교에 쓸 수 있는 건 `D635`뿐이다.** 2병렬 평균 100.9분은 zip 평균
        **1,081 MB** 프레임들에서 나온 값인데, `D635`는 1,204 MB로 비슷하지만
        `B5F3`은 **491 MB짜리 짧은 프레임**(관측 14초 vs 보통 25초)이라
        같은 자로 재면 안 된다. `B5F3` 시간은 참고치로만 적는다.
      - footprint 실측(08-19): `D635` 한반도교집합 **84.9%**(lon 125.92~129.08,
        lat 35.33~37.23) · `B5F3` **0.004%**(lon 127.87~130.76, lat 33.69~34.92,
        사실상 전부 바다). 둘 다 peninsula DEM 상자 안에 온전히 들어간다.
- [ ] 완료 후 압축률·비영값 전수 점검 → 2026-08 산출물 **32/32** 확인
...
- [ ] 🔄 **2병렬이 정말 순차보다 빠른지 잰다 — 측정 진행 중**(08-19 `D635`·
      `B5F3` 단독 실행). 그동안 아카이브에 **단독 실행 external DEM 측정치가
      하나도 없었다**(external DEM 배치는 전부 쌍으로 겹쳐 돌렸다). 2병렬 실측은 씬당 100.9분 = 처리량 **50분/씬**인데,
      [HW_UPGRADE_SPEEDUP_KR.md](../../XeonW-2123%284C8T%403.6GHz%29_32GM%20DDR4-266_GTX1080Ti/HW_UPGRADE_SPEEDUP_KR.md)
      는 **4물리코어라 병렬 이득이 1.3~1.6배에 그친다**고 본다. 게다가 2병렬은
      캐시를 `-c 14G → 7G`로 반 토막 내므로 HDD 재읽기가 는다.
      **씬 2장을 `-q 8 -c 14G` 단독 순차로 돌려 씬당 시간을 재면 결론이 난다.**
      남은 물량(2025-08 44씬 + 22~24년 92씬)에서 20% 차이면 수십 시간이다.
