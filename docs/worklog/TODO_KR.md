...
- [ ] **26년 7월 남한 궤도 19씬 VH ext-DEM** (진행 중, 배치 2개 병렬)
      — 대상: rel 32(o008355·o008530·o008705), rel 61(o008384·o008734 08EE),
        기타 SK 궤도의 미처리 프레임
- [ ] **기존 VH 44씬 재처리** (자동 DEM → external DEM). 파일명이 같아
      덮어쓰기이므로 **삭제 후 재생성**해야 배치가 건너뛰지 않는다.
...
- [ ] **GitHub 이슈 #8 본문 수정** — 잘못된 "계열 A" 사례가 아직 남아 있다.
      `gh issue edit <번호> --body-file temp/issues/08_common_footprint_mask.md`
- [ ] **이슈 #11·#12 등록** — `scripts/create_issues.ps1` 재실행(토큰 있는 셸)
- [ ] **대외 보고분 확인** — "25/26년 비교 가능한 상대궤도 짝이 하나뿐"이라는
      문장이 나갔다면 정정 필요. 실제로는 rel 54·61·127·134 네 궤도에 짝이
      있고 rel 134는 위성까지 같다.

## P0.5 — 8월 가뭄 비교쌍 (2026-08-07, 9/8 보고용)

배경·상세: [WORKLOG_20260807_KR.md](WORKLOG_20260807_KR.md).
광학은 2025-08 ↔ 2026-08인데 SAR만 7월이라 시기를 맞추는 작업.

- [x] **8월 비교쌍 선정** — 두 해 공통 상대궤도가 **ASC54 하나뿐**이라
      2025-08-06 ↔ 2026-08-02로 확정. 2025는 08-06/12/24 중 광학(08-01)과 가장
      가까운 08-06. **궤도를 섞으면 커버 영역이 달라져 면적 비교가 성립하지 않는다.**
- [x] **GRD 5장 다운로드** (`download_aug_pair.py` 신규) — 5/5 성공.
- [x] **VH RTC(Frost) 5장** — 7월과 동일 그래프(TF 포함·10 m·EPSG:4326·2밴드).
      성공 5 / 실패 0, 배치 2개 병렬 약 2시간 40분.
- [x] **모자이크** `mosaic_20250806_vh.vrt` · `mosaic_20260802_vh.vrt`
      (3/3장·2/2장 반영).
- [x] **커버리지 검증** (`check_mosaic_basin_cover.py` 신규) — 6개 유역 99.94%↑,
      **스와스 결측 없음**(결측이 수백 개로 흩어짐, 최대 덩어리 0.02 km²).
- [ ] **(다음, gee 폴더)** `watershed_pairs.py`의 `ORBIT` 표에 `"20250806":
      "ASC54"` · `"20260802": "ASC54"` 추가 → 쌍 재산정(병합, 덮어쓰기 금지) →
      `watershed_change.py`로 v9 산출(v8은 7월판이라 보존).
- [ ] **임계값 대조** — gee 산출 임계값이 두 해에 **1.5 dB 넘게 벌어지면**
      보고서에 위성 교체(2025 S1C ↔ 2026 S1D) 단서를 달 것. 사전 점검에서는
      남쪽 4개 유역이 ±0.15 dB라 계통 오프셋은 안 보이고, 임하만 P5 −1.44 dB.
- [ ] **섬진강·평림 처리 방침 결정** — ASC54 스와스 밖(교차 0%)이라 8월 SAR
      불가. 7월 값을 8월 표에 섞지 말 것. 보고서에서 제외하거나 시기를 명시.
- [ ] **2026-08 추가 촬영 확인** — 조사 시점(08-07)에 2026-08은 아직 1주일치다.
      월말에 ASC54가 더 촬영되면 광학과 더 가까운 쌍으로 다시 고를 여지가 있다.
- [ ] **8월 GRD zip 5장(5.7 GB) NAS 업로드 후 로컬 삭제** — F: 잔여 4.2 GB.
      RTC가 끝나 원본은 회수 가능. (사용자가 직접 수행 예정)
