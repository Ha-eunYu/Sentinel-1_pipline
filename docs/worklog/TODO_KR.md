# TODO (2026-08-26 갱신)

다른 컴퓨터에서 이어서 작업할 때 참고하는 실행 목록. 배경·근거는
[CODE_REVIEW_KR.md](../review/CODE_REVIEW_KR.md)(코드 품질)와 [PROGRESS_KR.md](PROGRESS_KR.md)
(작업 이력·데이터 현황)에 있고, 여기는 "무엇을 해야 하는지"만 체크리스트로 정리합니다.
문제의 원인·증상은 [ISSUES_KR.md](ISSUES_KR.md)에 따로 모읍니다.

> ## 읽기 전에 — 무엇이 정본인가 (2026-08-18 정리)
>
> **전처리 규격의 정본은 [PREPROCESSING_SPEC_KR.md](../pipeline/PREPROCESSING_SPEC_KR.md)
> 하나다.** DEM·편파·필터·궤도·해상도가 충돌하면 **사양서가 이긴다.** 이 문서
> 아래쪽에는 사양서 신설(08-18) **이전에 내린 결정**이 그대로 남아 있고, 일부는
> 이미 뒤집혔다. 뒤집힌 것은 그 자리에 ~~취소선~~과 ⚠ 로 표시해 두었다.
>
> **이 저장소의 범위는 RTC까지다**(2026-08-14 결정). 아래 섹션의 범위 구분:
>
> | 섹션 | 범위 | 상태 |
> | --- | --- | --- |
> | P0 · P0.1~P0.6 | ✅ RTC 본선 | 진행 중 |
> | P0.2 · P0.5 | 🔶 RTC 산출물을 넘기기 위한 설계·보고 | 진행 중 |
> | P1 · P1.5 · P1(구) · P2 · P3 | ⏸ **수체·침수 탐지 = 다른 프로젝트로 이관** | 기록 보존용 |
> | P4 · P5 | 🔧 이식성·구조 정리 | 언제든 |
>
> P1~P3의 미완 항목은 **이 저장소에서 하지 않는다.** 지우지 않고 남기는 이유는
> 인계받는 쪽이 "왜 여기서 멈췄나"를 알아야 하기 때문이다.

> **운영 규칙**: 작업 중 확인이 필요하거나 나중에 해야 할 것이 나오면
> **그때 바로 이 문서에 추가**한다. 대화에만 남기면 다음 세션에서 사라진다.

---

## 지금 열려 있는 것 (2026-08-26 전수 점검)

> 미체크 **52건**을 범위로 갈랐다.
>
> | 구분 | 건수 | 뜻 |
> | --- | ---: | --- |
> | ⏸ 이관 (P1 · P1(구) · P1.5 · P2 · P3) | 12 | **이 저장소에서 하지 않는다** — 수체·침수 탐지 |
> | 🔧 언제든 (P4 · P5) | 12 | 이식성·구조 정리. 급하지 않다 |
> | ✅ **RTC 본선** | **28** | 이 저장소가 해야 할 것 |
>
> 본선 28건 중 **P0.2(연도 간 비교 설계) 7건은 22~24년 확장이 선행**이라 지금은
> 손댈 수 없다. 나머지에서 **지금 바로 착수 가능한 12건**을 아래에 뽑았다.

### 🔴 먼저 — 코드 작업이 아니다

| # | 할 일 | 왜 지금인가 | 위치 |
| --- | --- | --- | --- |
| 1 | **CDSE 비밀번호 변경** | `.env` 가 public GitHub 에 커밋됐던 이력. 히스토리는 지웠지만 **노출은 되돌릴 수 없다.** 7/14 부터 열려 있다 | P0 |
| 2 | **S3 access/secret key 재발급** | 같은 `.env` 에 있었다 | P0 |
| 3 | 새 `.env` 를 프로젝트 **밖**에도 백업 | `git filter-repo` 가 로컬 `.env` 를 지운 사고가 있었다 | P0 |

### ✅ RTC 본선 — 처리·검증

| # | 할 일 | 상태 | 위치 |
| --- | --- | --- | --- |
| 4 | **북한 프레임 재처리 14씬** | ⏸ 배치 자리 대기. zip 14/14 보유 · 검사 적용됨 | P0.6(맨 아래) |
| 5 | **2026-07 잔여 3씬** `D298`·`3191`·`4C7C` | 🔄 **08-26 11:08 시작된 배치가 처리 중** | P0.1c |
| 6 | **2026-07 `07-03 rel 61` 1씬** 다운로드 + RTC | 7월 완결에 필요한 마지막 1장 | P0.4 |
| 7 | **2025-08 한반도 미처리 약 44씬** | 8월 비교의 나머지 절반. 약 110 GB | P0.4 |
| 8 | **7월분·2025년분 68씬 북부 결측 실측** | 재기 전에 단정하지 않는다 | P0.5b |
| 9 | `vh_coverage_report --zone kp --months 2026-08` 대조 | 32/32 완료분 교차확인 | P0.6 |

### 🟡 기록·판단 보류

| # | 할 일 | 언제 | 위치 |
| --- | --- | --- | --- |
| 10 | **POEORB 재구움 실측** | **09월 초 이후**(관측 +20일). 차이가 화소 이하면 재처리 없이 닫는다 | P0.6 |
| 11 | **산출물에 DEM·궤도출처 사이드카** | ISSUES #13·#17·#19 공통 잔여 | P0.6 |
| 12 | **한강 +4.0% 인용 판단** | 보고서 §6-3 을 읽고 가뭄 근거에서 뺄지 정할 것 | P0.5 |

**대량 확장(22~24년 92씬)은 위 7번까지 끝난 뒤에 연다.** 그때 F: 를 다시 비워야
한다(예상 130 GB).

---

## P0.1 — 25·26년 VH external DEM 통일 ✅ 완료 (2026-08-14 ~ 08-17)

배경: 7월 VH 산출물이 전부 `--dem` 옵션 도입(08-10) **이전**에 만들어져 SNAP
자동 캐시 COP30을 썼다. 하구 결측 문제([ISSUES_KR](ISSUES_KR.md) #2)에 노출돼
있어 external DEM(`downloads/dem_basin/korea_full_cop30.tif`)으로 통일한다.
**(⚠ 이 시점의 기준. 08-18 부터는 `korea_peninsula_cop30.tif` — 사양서 1-3절)**

- [x] **26년 7월 남한 궤도 19씬 VH ext-DEM** — 완료 (2026-08-14, 실패 0)
      — 대상: rel 32(o008355·o008530·o008705), rel 61(o008384·o008734 08EE),
        기타 SK 궤도의 미처리 프레임
- [x] **7월 전량 external DEM 통일 완료** (2026-08-17 02:05).
      25·26년 7월 63씬 모두 `korea_full_cop30.tif` 기준. 자동 DEM 잔여 0.
      재처리로 산출물이 커진 경우가 많다 — `F336`(25-07-01)은 **0.05 GB →
      1.16 GB**로, 자동 DEM일 때 대부분이 결측이었다(ISSUES #2의 극단 사례).
- [x] **비교 본선 4궤도 재처리 완료** (2026-08-15) — rel 134(25-07-18 C ↔
      26-07-20 C), rel 54(25-07-25 C ↔ 26-07-15 C) 17씬을 VH +
      korea_full_cop30 기준으로 다시 구웠다(**17/17 완료**, 08-15 05:58).
      `6D9F`는 `.SAFE.zip` 패턴 문제로 한 번 조용히 건너뛰어졌다가
      (ISSUES #15) 마지막에 처리했다 — 2.19 GB(자동 DEM) → 2.77 GB.
      **이 두 쌍은 수체탐지 프로젝트로 바로 넘길 수 있는 상태다.**
- [x] **8월 (25·26년) — 신규 3씬 + DEM 통일 재처리 6씬 완료** (2026-08-17)
      - 신규(VH 없음): `CDCC`·`037F`(25-08-11), `BDA2`(26-08-07)
      - **재처리(다른 external DEM을 씀)**: `2B35`·`9FBB`·`9F4F`(25-08-11),
        `EF16`·`FC19`(26-08-07) ← `korea_cop30.tif` /
        `17B9`(26-08-02) ← `han_cop30.tif`
      - 이미 `korea_full_cop30.tif`로 처리됨(재처리 불필요): `D0A4`·`E160`·
        `3EEB`(25-08-06), `9A11`·`C40B`(25-08-12), `3B05`·`B10F`(26-08-02)

> ## ⚠ 뒤집힌 결정 — 아래는 2026-08-14 기준이고 지금은 유효하지 않다
>
> **현행: 한반도 대상 처리는 전부 `korea_peninsula_cop30.tif`**
> ([사양서 1-3절](../pipeline/PREPROCESSING_SPEC_KR.md)). 아래 `korea_full` 통일
> 방침은 **대상이 남한이던 시절의 것**이다. 한반도로 넓히자 북위 39.9° 이북이
> 통째로 빠지는 문제가 드러났다(ISSUES #17). `korea_full`은 **남한 전용**으로만
> 남는다. 아래 표·문장은 P0.1 작업 당시의 근거 기록으로 보존한다.
>
> ~~**external DEM은 `korea_full_cop30.tif`로 통일한다(2026-08-14 결정).**~~
> ~~후보 DEM들의 실제 범위를 재보니 이것만 남한 전역 + 제주 + 울릉을 덮는다.~~
>
> | DEM | 범위 | 제주 |
> | --- | --- | --- |
> | `korea_full_cop30.tif` | 125.0~131.0E, 32.9~39.9N | ✅ |
> | `korea_cop30.tif` | 125.4~129.9E, **33.8**~38.7N | ❌ (제주 33.1~33.6N) |
> | `han_cop30.tif` | 125.7~129.9E, 35.6~39.2N | ❌ (부산·목포도 빠짐) |
>
> DEM 밖 영역은 RTC 산출물에서 **무효(결측)** 로 남는다. 유역 단위 작업에는
> 유역 clip DEM이 효율적이지만, **남한 전역을 한 기준으로 비교하려면 범위가
> 같아야** 한다.
>
> **용량 주의**: external DEM 산출물은 자동 DEM 것보다 크다(결측이 메워지므로).
> 7월 재처리로 F: 여유가 100 GB → 73 GB로 줄었다. 22~24년 확장 전에는
> NAS 업로드가 선행돼야 한다.

> ### ✅ 인계 준비 완료 (2026-08-17)
>
> | 시기 | 원본 zip | VH RTC | 자동 DEM 잔여 |
> | --- | ---: | ---: | ---: |
> | 2025-07 | 21 | 21 | 0 |
> | 2025-08 | 10 | 10 | 0 |
> | 2026-07 | 60 | **42** | 0 |
> | 2026-08 | 6 | 6 | 0 |
> | 합계 | 97 | **79** (147 GB) | **0** |
>
> 2026-07의 미처리 18씬은 **남한 커버 0%인 북한·해상 전용 궤도**다
> (754B·427D·74FD·F040·2B06·C9CC·DF80·32ED·1571·5D47·525F·9FFF·D298·
> 3191·4C7C·0B91·3194·639F). 남한 우선 방침에 따라 의도적으로 제외했고,
> 북한 분석 단계에서 처리한다(P0.4).
>
> **이 프로젝트의 범위는 RTC까지다(2026-08-14 결정).** VH external DEM RTC가
> 끝나면 여기서 마친다. **수체탐지는 다른 프로젝트에서 진행**하므로 이 저장소의
> `build_water_per_date_otsu.py` 재실행은 계획에 넣지 않는다.
> 넘길 것: `downloads/rtc_grd_frost_vh/*_rtc_db_vh.tif`(전량 external DEM),
> 처리 조건은 [PROCESS_202507_202607_KR.md](../pipeline/PROCESS_202507_202607_KR.md)
> 2절, 궤도 선별 근거는
> [RELATIVE_ORBITS_KR.md](../pipeline/RELATIVE_ORBITS_KR.md).

## P0.1b — VV·자동 DEM 산출물 정리 ✅ 완료 (2026-08-17)

VH external DEM 통일이 끝나 **VV RTC를 삭제**했다. 176 GB 확보(F: 67 → 243 GB).

| 폴더 | 삭제 | 내용 |
| --- | ---: | --- |
| `downloads/rtc_grd/` | 61 tif, 82.8 GB | VV · Refined Lee · 자동 DEM |
| `downloads/rtc_grd_frost/` | 71 tif, 93.3 GB | VV · Frost · 자동 DEM |

**보존한 것**(사용자 결정): `gtc/`(81.9 GB, RTC_VS_GTC 근거), `water_otsu/`
(수체 지도 + 임계값·면적 CSV), `rtc/`, `rtc_extdem/`, `excluded_china_japan/`,
벤치마크.

### 삭제로 생긴 결과 — 알고 있어야 할 것

- **VV 기반 7월 Otsu 결과는 재현하려면 재처리가 필요하다.** 수치 자체는
  `water_otsu/otsu_thresholds.csv`·`water_area_perrow.csv`(git 추적)와
  [WATER_AREA_KR.md](../water/WATER_AREA_KR.md)에 남아 있다.
- **`downloads/rtc_grd/*.vrt` 23개는 죽은 링크가 됐다**(참조하던 tif가 없다).
  QGIS에서 열면 오류가 난다. 삭제하지 않고 어떤 조합의 모자이크였는지 기록으로
  남겼다.
- **2026-06 12씬 + 2026-07 3씬(6EBE·BB45·E265)은 원본 zip도 없어 로컬에서
  완전히 사라졌다.** 필요해지면 CDSE에서 재다운로드해야 한다.
  6월 처리 재개 시 이 점을 먼저 확인할 것.
- `downloads/rtc_grd/get_tif_shooting_area.py`는 데이터 폴더에 있던 도구라
  삭제 전에 `s1/tools/audit/tif_extent_report.py`로 옮겼다.

## P0.1c — 26년 7월 VH ext-DEM 재개 대기 (2026-08-19 중지)

다른 작업에 자원이 필요해 **사용자 요청으로 중지**. 설정 변경 없이 그대로 재개한다.

**중지 시점 상태**

- 26년 7월 VH 산출물 **43개**. 이번 배치에서 `754B`(07/03) 1씬 완료.
- `525F`(27% 기록) · `1571`(0%)은 **미완성이라 삭제**했다. 남겨두면 다음 실행이
  "이미 처리됨"으로 조용히 건너뛴다(ISSUES #15와 같은 함정).
- 남은 미처리 **18씬**. 실행 중 프로세스 없음. F: 여유 174 GB.

> ### ▶ 재개했다 — 18씬 중 **15씬 완료, 3씬 남음** (2026-08-26 13:30 확인)
>
> 08-25 08:31 에 재개해 08-25 21:51 `32ED` 까지 14씬을 냈고, **08-26 11:08 에
> 새 배치가 다시 걸려 2병렬로 돌고 있다.**
>
> | 상태 | 씬 |
> | --- | --- |
> | 완료 15 | `427D`·`74FD`·`F040`·`2B06`·`C9CC`·`DF80`·`32ED`·`1571`·`5D47`·`525F`·`9FFF`·`0B91`·`3194`·`639F`·`CE22` |
> | 🔄 남음 3 | **`D298`·`3191`·`4C7C`** |
>
> **08-25 에 나온 12장은 `rtc_integrity` 로 점검 완료 — 하한 미달 0**
> (유효 42.9~66.8%). 남은 3씬도 나오면 같은 명령으로 볼 것.
> ⚠ 이 배치들은 **유효화소 검사가 들어가기 전에 시작**됐다(실행 중인 파이썬이
> 옛 코드를 물고 있다). 끝나면 반드시 도구로 점검한다.
>
> 📌 **북한 재처리(P0.6 맨 아래)는 이 배치가 끝나야 시작할 수 있다** — 지금
> 걸면 4병렬이 되어 RAM 32 GB 를 넘긴다.

**재개 명령** (확정 사양 그대로 — VH · Frost · peninsula DEM · 2병렬)

```powershell
# 배치 A (9씬)
conda run -n s1_snappy python -m s1.tools.preprocess.batch_grd_rtc_frost `
  --month 202607 --pol VH --out-dir downloads/rtc_grd_frost_vh --out-tag _vh `
  --dem downloads/dem_basin/korea_peninsula_cop30.tif --gpt-c 7G --oldest-first `
  --only "427D,74FD,F040,2B06,C9CC,DF80,32ED,1571,5D47" `
  *> temp/logs/_rtc_202607_kp_a.log

# 배치 B (9씬)
conda run -n s1_snappy python -m s1.tools.preprocess.batch_grd_rtc_frost `
  --month 202607 --pol VH --out-dir downloads/rtc_grd_frost_vh --out-tag _vh `
  --dem downloads/dem_basin/korea_peninsula_cop30.tif --gpt-c 7G --oldest-first `
  --only "525F,9FFF,D298,3191,4C7C,0B91,3194,639F,CE22" `
  *> temp/logs/_rtc_202607_kp_b.log
```

배치는 완료분을 건너뛰므로 **같은 명령을 다시 돌려도 안전**하다. 예상 약 15시간
(2병렬 시간당 1.2씬).

**중지 후 반드시 확인할 것** — 강제 종료하면 쓰다 만 tif 가 남는다. 크기가
`가로×세로×4바이트`의 40% 미만이면 미완성으로 보고 지운다(정상은 70% 안팎).

**이 다음 순서**: 25년 7월 6씬 → 25년 8월 44씬.

## P0.2 — 연도 간 비교 설계 (2026-08-14 확정, 실행 대기)

배경·근거: [RELATIVE_ORBITS_KR.md](../pipeline/RELATIVE_ORBITS_KR.md).

- [ ] **공통 관측영역 마스크** — (대권역 × 상대궤도)를 단위로, 양쪽 다
      유효화소인 픽셀만 남겨 집계. 한강처럼 한 궤도가 다 못 덮는 대권역은
      궤도별 부분면적으로 나눠 비교하고 **합산 대신 궤도별 변화율**을 주 지표로.
- [ ] **궤도 간 편향 측정** — 동일일자 6쌍(2023-08-04 rel54↔127 등)의 공통
      영역 수체 면적 차이. 판정: <5% 합산 가능 / 5~15% 보정 후 사용 /
      >15% 궤도별 따로 제시([RELATIVE_ORBITS_KR](../pipeline/RELATIVE_ORBITS_KR.md) 7-6).
      → 2023년·2025년 8월 일부는 **원본 미보유**라 다운로드가 선행.
- [ ] **rel 134·61의 23·25·26년 3개년 비교**
- [ ] **⚠ rel 61 분리도 확인** — 25년 관측이 1프레임짜리 2건뿐이고 남한 커버가
      8~29%로 얕다. 25년 7/19 o060151이 Otsu 실패로 fallback(−16 dB)한 것과
      같은 조건이다. **결과가 나오면 임계값보다 분리도 η를 먼저 보고**, 낮으면
      비교에서 제외하거나 fallback 여부를 명시할 것.
- [ ] **22~24년 확장은 rel 54·127로만** (5개년 연속 관측되는 유일한 궤도)
- [ ] **2025년 S1C를 쓸 때는 그 달 오프셋을 STAC으로 확인**하고 시작
      (`relative_orbit_survey.py --years 2025 --months ...`). 표의 171은
      7·8월에서만 확인한 값이다.
- [ ] 제주 전용 COP30 clip → rel 134 씬으로 제주 RTC (현재 보류)
- ~~SLC 컬렉션 상대궤도 조사~~ — **불필요, 제외(2026-08-14)**.
  수면적 산출의 입력은 GRD다. SLC는 **간섭(coherence·InSAR)** 을 할 때만 필요한데
  이 프로젝트 범위가 아니다. 조사 도구에 `--collection` 옵션이 있어 관성으로
  적어둔 항목이었다. 나중에 coherence 분석을 하게 되면 그때
  `relative_orbit_survey.py --collection sentinel-1-slc` 한 줄로 확인한다.

## P0.3 — 정정 전파 (2026-08-14)

- [x] **GitHub 이슈 본문 정정** — 정정본으로 #8 재등록 완료(구버전 #6은 닫을 것)

- [x] **이슈 #11·#12 등록 완료** — GitHub #9·#10 (2026-08-14)
- [x] **GitHub #6 닫기(#8 중복) 완료** (2026-08-14) — 제목 변경으로 중복 생성됐던 건

- [ ] **대외 보고분 확인** — "25/26년 비교 가능한 상대궤도 짝이 하나뿐"이라는
      문장이 나갔다면 정정 필요. 실제로는 rel 54·61·127·134 네 궤도에 짝이
      있고 rel 134는 위성까지 같다.

## P0.4 — VH ext-DEM 미처리 전수 조사 결과 (2026-08-17)

CDSE 관측 목록(`data/relative_orbits_sentinel-1-grd.csv`, 22~26년 7·8월 한반도
교차 씬) 대비 로컬 보유·처리 현황. **키는 관측 시작시각+절대궤도**로 맞췄다 —
씬 ID 끝 4자리는 제품 해시라 같은 촬영도 다르게 센다(ISSUES #16).

| 연월 | 남한 VH완료 | 남한 로컬·미처리 | 남한 원본없음 |
| --- | ---: | ---: | ---: |
| 2022-07 / 08 | 0 / 0 | 0 / 0 | **13 / 13** |
| 2023-07 / 08 | 0 / 0 | 0 / 0 | **16 / 11** |
| 2024-07 / 08 | 0 / 0 | 0 / 0 | **15 / 24** |
| 2025-07 / 08 | **16** / 9 | 0 / 0 | 0 / **25** |
| 2026-07 / 08 | **28** / 6 | 0 / 0 | **1** / **6** |

- **7월은 사실상 끝났다.** 2025-07 남한 16/16, 2026-07 남한 28/29(빠진 건
  `07-03 rel 61` 1씬뿐).
- **구멍은 8월이다.** 2025-08 남한 25씬, 2026-08 남한 6씬이 원본 자체가 없다.
  (로컬 8월분은 특정 비교쌍만 받아둔 것이라 그렇다.)
- **22~24년은 남한 92씬 전량 미보유.** 장기 확장은 다운로드부터다.
- 2026-07 북한·해상 15씬은 로컬에 있으나 남한 우선 방침으로 미처리.

### 할 일

- [ ] **2026-07 `07-03 rel 61` 1씬** 다운로드 + VH ext-DEM (7월 완결)
- [x] ~~**2026-08 6씬** 다운로드~~ — **재조사 결과 실제 부족분은 1씬뿐이었다**
      (2026-08-17). `B5F3` 하나를 받아 2026-08 한반도 원본은 32/33이 로컬에
      있다(나머지 `8B48`은 한반도 교집합 0.39%라 대상 밖). 위 표의 "원본없음 6"은
      **남한 기준 집계**였고, 그중 대부분은 이미 로컬에 있었다 → P0.6
- [ ] **2025-08 한반도 미처리 약 44씬** VH ext-DEM
      → 8월 비교(rel 54·61·127·134)의 나머지 절반. 특히 **rel 134의
      25-08-11·08-23**이 26-08-13(KST 8/14)과 **위성까지 같은 짝**인데 미처리다.
- [ ] **22~24년 남한 92씬** — rel 54·127만 추리면 절반 이하로 줄어든다.
      다운로드 용량 약 90~100 GB, RTC 산출물 약 130 GB 예상.
      **E: 여유 812 GB**에 받고 F:는 산출물만 쓰는 게 안전하다.
- [x] **조사 도구 고정 완료** — `s1/tools/audit/vh_coverage_report.py`.
      단, 입력 CSV(`data/relative_orbits_sentinel-1-grd.csv`)는 **2026-08-12까지**라
      최신 관측을 보려면 `relative_orbit_survey.py`를 먼저 다시 돌려야 한다.

## P0.5b — DEM 확정과 북부 결측 확인 (2026-08-18)

전처리 규격을 [PREPROCESSING_SPEC_KR.md](../pipeline/PREPROCESSING_SPEC_KR.md)로
확정했다. **한반도 전체를 덮는 DEM은 `korea_peninsula_cop30.tif` 하나뿐이다**
(123.5~131.3E, 32.8~43.3N, 1.6 GB).

- [x] **결측 판정 방법 확정** — 가정하지 말고 **씬별로 잰다**.
      `(footprint ∩ 한반도)` 중 옛 DEM 상자 **밖** 면적 비율을 계산해 1% 이상만
      재처리 대상으로 본다. `s1.core.aoi.footprint_ring` + shapely 몇 줄이면 된다.
- [x] **2026-08분(32씬) 결측 실측 완료** (2026-08-18) — 결과가 반가웠다.

      | 손실 | 씬 | 비고 |
      | --- | ---: | --- |
      | < 1% | 15 | 이 중 **VH 완료 11장 전부** → 재처리 불필요 |
      | 1~80% | 9 | `A949` 80.7% · `524F` 79.0% · `F0C3` 47.2% · `7C24` 17.9% |
      | 100% | 8 | `226A`·`31FB`·`E6FC`·`65AB`·`9A5C`·`B808`·`A54D`·`7AE5` |

      **손실 ≥1%인 17씬은 전부 미처리분이었다.** 즉 8월분에 한해 재처리는 0건이고,
      "DEM을 바꿨으니 다 다시"라는 반사적 판단이 약 5시간의 헛일이 될 뻔했다.
- [ ] **7월분·2025년분(나머지 68씬) 북부 결측 실측** — 같은 방법으로. 남한 궤도
      위주라 대부분 1% 미만일 것으로 보이나, **재기 전에는 단정하지 않는다.**
- [ ] 결측 1% 이상인 씬만 `korea_peninsula_cop30.tif`로 재처리
      (`scripts/rebake_vh_extdem.ps1` — 배치가 건너뛰므로 **삭제 후 재실행**)
- [x] **앞으로 처리하는 씬은 `korea_peninsula_cop30.tif`** — 2026-08 잔여 20씬부터
      적용 중(P0.6)
- [ ] 산출물에 사용 DEM 을 기록하는 방법 검토(현재는 로그를 뒤져야 안다.
      ISSUES #13·#17의 공통 잔여 과제)

## P0.6 — 2026년 8월 한반도 전량 전처리 (2026-08-18 진행 중)

대상을 남한 → **한반도**로 넓혔다. 근거 달력은
[ORBIT_CALENDAR_202607_08_KR.md](../pipeline/ORBIT_CALENDAR_202607_08_KR.md),
경위는 [WORKLOG_20260818_KR.md](WORKLOG_20260818_KR.md).

| 구분 | 씬 | DEM | 상태 (08-18 08:00) |
| --- | ---: | --- | --- |
| 한반도 교집합 ≥ 1% | **31** | — | 처리 대상 |
| ├ 완료 | 19 | 혼합(아래) | ✅ 무결성 전수 점검 통과 |
| ├ 진행 | 2 | `korea_peninsula_cop30.tif` | 🔄 `D2A0`·`31FB` |
| ├ 대기 | 11 | `korea_peninsula_cop30.tif` | ⏳ 배치 큐 |
| └ 재처리 대기 | 1 | `korea_peninsula_cop30.tif` | ⚠ `D635` — 아래 참고 |
| 한반도 교집합 < 1% | 2 | — | ⏸ 제외 (`B5F3` 0.00% · `8B48` 0.39%) |

> **⚠ `D635` 사고** — 08-18 01:26 배치를 강제 종료할 때 처리 중이던 씬이라
> **값이 전부 0인 0.15 GB 껍데기**가 남았다. 파일이 있으니 배치가 건너뛰어
> 재실행으로도 안 고쳐진다. 삭제 후 큐에 다시 넣었다. 하필 08-13 rel 134에서
> **남한 커버가 가장 넓은(85.6%)** 프레임이다.
>
> **판정은 압축률로 한다.** 정상 19장은 무압축 대비 51.8~87.4%인데 `D635`는
> **5.1%**였다(값이 0이면 deflate가 극단적으로 잘 먹는다). 용량만으로는
> 못 가른다 — 정상 산출물도 1.72~2.85 GB로 폭이 넓다.

- [x] **배치 2개(`_kp_c`·`_kp_d`) 완료** — 각 10/10, **실패 0 · 건너뜀 0**
      (08-18 17:4x). 2026-08 한반도 산출물 30/32.
- [x] ✅ **`D635` 재처리 완료** (2026-08-19 09:18, 단독 **80.0분**, **2.58 GB**).
      무결성 확인 — 압축률 **43.2%**(정상대 25~44%), 유효화소 **70.9%**,
      dB 중앙값 −15.4(p1 −48.4 / p99 −6.4). 01-26 사고 때의 껍데기
      (0.15 GB · 압축률 5.1% · 비영값 0%)와 완전히 다르다. **2026-08 = 31/32.**
- [x] ✅ **`B5F3` 완료** (2026-08-19 10:06, 단독 **32.5분**, **1.25 GB**).
      압축률 35.5% · 유효 59.7% — 정상. ⚠ **분석에 쓰지 말 것**: dB 중앙값이
      **−31.7**(D635 는 −15.4)로 전형적인 **잔잔한 바다** 값이다. footprint
      실측 한반도교집합 0.004%와 일치한다. 선별 기준(1% 규칙)은 그대로 유효.
- [x] ✅ **2026-08 한반도 전처리 32/32 완료** (배치 완료: 성공 2 / 실패 0)
- [x] 압축률·비영값 점검 완료 — 32장 전부 정상(압축률 25.8~43.7%). 불량 0
- [ ] 완료 후 `vh_coverage_report --zone kp --months 2026-08`로 대조
      (⚠ 아래에 같은 항목이 한 번 더 있었다 — 2026-08-26 합쳤다)
- [x] ~~배치 2개 잔여 2씬 완료 확인~~ (완료: `524F` 92.2분 · `226A` 119.4분)
      — 실측 씬당 **71~128분**(평균 **100.9분**, 중앙값 101.3, n=20). 2병렬 = **시간당 1.2씬**.
      최초 "30분/씬" 추정이 빗나간 원인은 **큰 DEM도 북부 프레임도 아니다** —
      그 30분이 자동 DEM 시절 **깨진(결측투성이) 산출물**에서 나온 값이었다.
      근거표: [PREPROCESSING_SPEC_KR.md](../pipeline/PREPROCESSING_SPEC_KR.md) 2-1절
- [x] ✅ **강제 종료 후 전수 점검이 도구로 고정됐다** (2026-08-25) — 눈으로
      "부분 산출물 없음"을 확인하는 방식은 실패한 적이 있다(엉뚱한 씬을 봤다).
      이제 `python -m s1.tools.audit.rtc_integrity` 가 유효화소·압축률을 표로
      찍고, **배치가 씬마다 자동으로 검사**한다(ISSUES #24).
- [x] ✅ **압축률 QC 기준의 계산식 문서화 완료** (2026-08-25) — 손으로 계산한
      수치를 문서에 박지 않는다. 계산은 [rtc_qc.py](../../s1/core/rtc_qc.py)
      `measure()` 한 곳에서 하고 자료형은 `rasterio` 에서 읽는다. 기준표는
      [사양서 6-1절](../pipeline/PREPROCESSING_SPEC_KR.md). ISSUES #20 🟢.
- [ ] 🟡 **POEORB 없이 RESORB로 처리된 씬 정리** — 2026-08분 **18씬 전량**이
      정밀궤도 미공개 상태에서 처리됐다(`continueOnFail=false`는 못 막는다).
      7월분도 일부 해당(`_vh_jul26_b` 6씬, `_vh_re26` 2씬). POEORB는 관측 +20일
      공개이므로 **09월 초 이후** 같은 씬 1장을 POEORB로 다시 구워 기하 차이를
      실측하고, 그 결과로 전량 재처리 여부를 결정한다.
      상세: [PREPROCESSING_SPEC_KR.md](../pipeline/PREPROCESSING_SPEC_KR.md) 2-2절
- [ ] 🟡 **산출물에 `DEM`·`궤도 출처`를 사이드카로 남기기** — 지금은 둘 다
      로그를 뒤져야 안다. ISSUES #13·#17의 잔여 과제와 같은 건이다.
- [ ] **`8B48` 처리 여부 결정** — 미보유. 한반도 교집합 0.39%(러시아·동해)라
      1% 규칙으로 뺐다. 북한 동해안을 다룰 때 필요하면 받는다.
- [x] ✅ ~~🔴 F: 여유 180 GB — 다음 대량 처리 전에 반드시 비운다~~ — **해소**
      (2026-08-26 확인 **F: 228 GB · E: 713 GB**). `gtc`·`water`·
      `excluded_china_japan` 정리로 136 GB 를 확보했다(커밋 `7fc976b`).
      2025-08 잔여 44씬(약 110 GB)은 지금 여유로 들어간다. **22~24년 92씬
      (예상 130 GB)까지 이어서 가려면 그때 다시 비워야 한다.**
- [ ] 완료분 NAS 업로드 후 로컬 zip 정리
- [x] **2병렬이 순차보다 빠른지 측정 완료** (2026-08-19, `D635` 단독).
      **단독 80.0분/씬 vs 2병렬 처리량 53.2분/씬 → 2병렬이 1.5배 빠르다.**
      크기 보정 후 씬당으로는 병렬이 1.33배 느리지만 두 장이 동시에 나온다.
      HW 문서의 실효 병렬 예측(1.3~1.6배)과 일치. **2병렬 유지가 맞다.**
      (단독 표본 n=1 — 다음 단독 실행 때 재확인)
## P0.5 — 8월 가뭄 비교쌍 (9/8 보고용)

배경·상세: [WORKLOG_20260807_KR.md](WORKLOG_20260807_KR.md)(최초 6개 유역) ·
[WORKLOG_20260810_12_KR.md](WORKLOG_20260810_12_KR.md)(유역 확장 + external DEM).
광학은 2025-08 ↔ 2026-08인데 SAR만 7월이라 시기를 맞추는 작업.

### 전처리 (S1) — 완료

- [x] **8월 관측 전수 점검** — 78장을 (촬영일×상대궤도)로 묶어 8개 댐유역·
      대권역 커버리지 산정. 8/7의 "6개 유역만 가능" 결론이 뒤집혔다(2026-08-07
      ASC127이 조회 후 등재됨).
- [x] **6개 댐 유역 + 낙동강** ASC54 2025-08-06 ↔ 2026-08-02 (1C↔1D)
- [x] **섬진강·영산강·평림** ASC127 2025-08-11 ↔ 2026-08-07 (1C↔1D)
- [x] **금강** ASC127 같은 쌍 — 9F4F 1장 추가로 76.5% → **100%**
- [x] **한강** ASC54 **2025-08-12** ↔ 2026-08-02 (**1A↔1D**) — 공통 85.1%.
      08-06(1C)은 69.3%가 상한이라 커버리지를 택했다.
- [x] **external DEM 전환** — ASC54 5장 재처리로 8월 14장 전량 통일.
      자동 캐시 DEM이 낙동강 하구 **12.48 km²**를 무효로 만들던 결함 제거.
      창 패치는 불필요해져 `rtc_extdem/_superseded/`로 격리.
- [x] **`korea_full_cop30.tif`** — 대권역 21 + 댐유역 8 전부 nodata 0.
- [x] **GRD 원본 E: 이관**(169 GB, 정션) — F: 4.2 → 173.4 GB.

### 산출 (gee) — 진행 중

- [x] `watershed_change_v9.csv` (낙동강 + 6개 댐) · `_v9_asc127.csv`
      (섬진강·영산강·평림). external DEM 반영으로 임계 +0.70/+0.53 dB 이동,
      변화율 최대 4.5%p 변동(성덕 −29.7 → −25.2%).
- [x] ✅ **한강·금강 산출 추가 완료** (2026-08-26). v9 두 파일에 두 유역 행이
      없었던 건이다.
      - [x] ~~`ORBIT` 표에 `"20250812": "ASC54"` 등록~~ — **이미 돼 있었다**
            (2026-08-25 확인). 표는 `watershed_pairs.py` 가 아니라
            **`orbit_vrt.py` 한 곳**으로 합쳐졌고 `("20250812","060501"):
            "ASC54"` 가 `SAT` 까지 포함해 등록돼 있다.
      - [x] ✅ **진짜 선행조건은 `watershed_pairs_aug.json` 이었다** (2026-08-25 해결)

> ### ⚠ 8월 쌍 파일이 조용히 두 유역을 건너뛰게 하고 있었다
>
> `watershed_change.py` 는 `--force-d2025/--force-d2026` 로 날짜를 강제해도
> **커버리지는 쌍 파일의 `cover_all` 에서 읽는다.** 거기 없는 날짜는 `0.0` 이
> 되고, `cover < 0.05` 면 `🔴 강제 날짜 … 커버 0% — 건너뜀` 으로 빠진다.
> **오류가 아니라 한 줄 경고라 놓치기 쉽다.**
>
> `watershed_pairs_aug.json`(2026-08-10 작성)은 유역마다 담긴 날짜 집합이 달랐다
> — 부분 실행이 병합되며 쌓인 결과다.
>
> | 유역 | 필요한 쌍 | 쌍 파일에 있었나 |
> | --- | --- | --- |
> | 한 강 | ASC54 `20250812` ↔ `20260802` | **0812 없음** → 커버 0% |
> | 금 강 | ASC127 `20250811` ↔ `20260807` | **둘 다 없음** → 커버 0% |
> | 섬진강·영산강 | ASC127 같은 쌍 | 있음(그래서 산출됐다) |
>
> 게다가 남아 있던 값도 낡았다 — VH 모자이크가 **2026-08-21 에 external DEM 으로
> 다시 만들어졌는데** 쌍 파일은 그 전 것이다. 한강 `20260802` 가 실측 **43.4%
> → 84.9%** 로 두 배 가까이 올랐다.
>
> **조치** — 두 유역의 `cover_all` 만 현재 모자이크 기준으로 다시 채웠다
> (36개 날짜 전량). 낙동강·섬진강·영산강 행은 **일부러 손대지 않았다** — v9
> 산출의 근거라 값이 바뀌면 그 회차를 재현할 수 없다.
> 백업: `watershed_pairs_aug_20260810.json.bak`
>
> | 유역 | 2025 | 2026 | 공통(min) |
> | --- | ---: | ---: | ---: |
> | 한 강 | 08-12 **84.8%** | 08-02 **84.9%** | **84.8%** |
> | 금 강 | 08-11 **100.0%** | 08-07 **100.0%** | **100.0%** |
>
> 📌 **일반 규칙**: 쌍 파일은 모자이크를 다시 만들면 **같이 낡는다.** 커버가
> 이상하게 낮으면 산출을 의심하기 전에 **쌍 파일의 작성 시각과 모자이크
> 작성 시각을 대조**할 것.
- [x] ✅ **한강·금강 산출 완료** (2026-08-26)

      | 유역 | 쌍 | 제약km² | 전 | 후 | Δ% | 산출범위 |
      | --- | --- | ---: | ---: | ---: | ---: | --- |
      | 금강 | ASC127 0811↔0807 | 467.60 | 172.49 | 168.13 | **−2.5%** | 제약적용 |
      | 한강 | ASC54 0812↔0802 | 971.75 | 269.06 | 279.87 | **+4.0%** | 제약적용(공통 79%) |
      | 한강(대조) | 〃 | +북쪽 7,058 | 493.34 | 493.03 | −0.1% | 제약+북쪽AOI |

      `output/watershed_change_v9_gum.csv` · `_v9_han_con.csv` · `_v9_han.csv`
- [x] ✅ **한강 산출범위 표기 — 두 판을 다 냈다.** 제약적용 판이 다른 유역과
      비교되는 값이고, "제약+북쪽AOI" 판은 대조용이다. 북쪽 7,058 km²는 제약
      자료가 없어 **논 담수가 들어가며**, 소실 87.22 ↔ 신규 86.91로 상쇄돼
      −0.1%가 된다 — "변화 없음"이 아니라 "논 때문에 상쇄됨"이다.

> ⚠ **`커버` 와 `산출범위` 는 다른 축이다 — 섞어 쓰지 말 것.**
>
> | 열 | 무엇을 말하나 | 무엇이 정하나 |
> | --- | --- | --- |
> | `커버` | **관측이 닿았나** (두 날짜 공통 유효화소) | 스와스 기하 · DEM 범위 |
> | `산출범위` | **기준자료가 수록됐나** | V-world 북단 (`reference_covered()`) |
>
> 한강이 `제약+북쪽AOI` 인 것은 **미관측 21.1% 때문이 아니라** 유역 북부가
> V-world 수록 범위 밖이기 때문이다. 원인도 대상도 다르고, 한강은 마침
> 둘 다 해당돼 겹쳐 보일 뿐이다. **미관측률로 이 라벨을 붙이면 gee 이슈
> #41**(라벨과 판정을 한 문자열에 담아 사전 조회가 빗나간 사고)**이 재발한다.**
> 보고서에는 두 값을 나란히 적는다.
>
> 경계선 자체의 이력·미해결 건: [gee BORDER_BOUNDARY_KR.md](../../../gee/Korea_WaterDetection_2025_2026/BORDER_BOUNDARY_KR.md)
- [ ] 🔴 **한강 +4.0% 은 9개 대상 중 유일한 증가다 — 인용 전 확인할 것**

> ### 한강 증가가 임계값 인공물인지 실측으로 갈랐다 (2026-08-26)
>
> 한강만 두 해 임계가 **밝은 쪽으로** 움직였다(−20.68 → −20.32). 다른 유역은
> 같거나(섬진강·영산강) 어두워졌다(낙동강 −0.17 · 금강 −0.36). 임계가 밝아지면
> 그것만으로 물이 늘어 보인다.
>
> 그래서 두 해 dB **분포 자체**를 같은 격자·공통유효역(29,169 km²)에서 쟀다.
>
> | 분위 | 2025 S1A | 2026 S1D | 차 |
> | ---: | ---: | ---: | ---: |
> | 1% (물) | −24.77 | −25.13 | **−0.36** |
> | 50% (뭍) | −14.41 | −14.05 | **+0.36** |
> | 99% | −7.77 | −7.43 | +0.34 |
>
> **뭍 분포가 통째로 +0.32~0.42 dB 밝아졌고 임계도 정확히 같은 폭으로 따라갔다.**
> 두 해 모두 임계가 **뭍 중앙값보다 6.27 dB 아래**라는 같은 자리다 — Otsu가 계통
> 이동을 상쇄했으므로 **임계 이동 자체는 면적을 부풀리지 않았다.** 물 꼬리는
> 반대로 0.36 dB 어두워져 분리가 0.72 dB 넓어졌고, 증가분은 거기서 나온다.
>
> **그래도 남는 한계 3가지** — 보고서 §6-3에 적었다.
> ① 관측률 79%(임진강·한강하구 북서쪽이 빠졌다) ② 한강만 **1A↔1D**라 +0.36 dB
> 이동에 위성차와 토양수분이 섞여 분리되지 않는다 ③ η 0.66/0.69로 상속 기준
> 미달(부모가 없어 자체 임계).
>
> 📌 가뭄 판정 근거 유역에서는 **빼는 편이 안전하다** — 남은 79%만 대표한다.

### 보고서

- [x] ✅ **[REPORT_AUG_SAR_KR.md](../../../2026_drought_map/REPORT_AUG_SAR_KR.md) 갱신**
      (2026-08-26) — 표 5의 금강·한강 두 행을 채우고, §6-3 신설(한강 증가에
      대한 분포 실측·한계 3가지), §7 산출근거·재현명령 추가, 머리말 상태표 갱신.
      ⚠ 이 폴더는 **git 저장소가 아니다.** 백업을 떠 두었다 —
      `REPORT_AUG_SAR_KR.md.20260826.bak`
      - **§3·표 6의 "재산출 대기 / 잠정"은 이미 해소돼 있었다**(2026-08-14 갱신분).
        이 TODO 항목이 낡아 있었던 것이다.
      - `_reorbit` 판(궤도별 모자이크 재현본)은 v9 대비 Δ% 최대 0.2%p 라
        **어느 쪽을 인용해도 결론이 같다**(`WATERSHED_RUN_HISTORY_KR.md` §재현 검증).
        §7에 그 사실을 적어 뒀다.
- [x] 임계값 대조 — 두 해 차이 0.17 dB로 1.5 dB 선에 크게 못 미친다.
      위성 교체 단서는 불필요(단 한강은 1A↔1D라 별도 표기).
- [x] 섬진강·평림 방침 — ASC127로 산출 가능해져 8월 표에 포함.

### 남은 확인

- [ ] **2026-08 재조회** — 8/7에 "없다"고 판정한 뒤 ASC127이 등재된 전례가
      있다. 한강을 1C↔1D로 되돌릴 짝이 생겼는지 확인할 것.
- [ ] **8월 GRD zip NAS 업로드 후 로컬 삭제** (사용자 수행 예정)

## P0 — 지금 바로 (다른 작업보다 먼저)

- [ ] **CDSE 비밀번호 변경**. `.env`가 한동안 public GitHub에 커밋돼 있었고
      (`git filter-repo`로 히스토리에서는 제거했지만 노출 자체는 되돌릴 수 없음),
      실제 `CDSE_PASSWORD` 값이 노출됐었습니다.
      <https://dataspace.copernicus.eu> 계정 설정에서 변경.
- [ ] **S3 access/secret key 재발급**. 같은 `.env`에 `CDSE_S3_ACCESS_KEY`,
      `CDSE_S3_SECRET_KEY`도 있었습니다. 발급받은 콘솔에서 폐기 후 재발급.
- [ ] 두 값을 로컬 `.env`(현재 이 컴퓨터)와, 다른 컴퓨터로 옮길 경우 그쪽 `.env`에도 갱신.
- [ ] **(신규, 7/14) `.env` 재발급 시 반드시 백업해둘 것**: 오늘 `git filter-repo`
      실행 중 로컬 `.env`가 통째로 삭제되는 사고가 있었음(아래 P4 참고). 지금
      만드는 새 `.env`는 프로젝트 밖(예: 비밀번호 관리자, 클라우드 노트)에도
      한 부 저장해두면 같은 사고가 나도 git 작업만으로 복구 가능.

## P1 — 한반도 전체 수집·탐지 (진행 중, 2026-07-20 갱신)

- [x] **일본/중국 전용 궤도 제외** — `Korea_Peninsula.geojson` bbox 검색이라
      가장자리에 일본·중국 위주 프레임이 섞여 들어옴. 판별은 **프레임 전체
      footprint 폴리곤을 `Korea_Peninsula.geojson`(남북한 실경계)과 shapely로
      교차 계산**해서 함 (bbox 사각형 겹침이나 대표좌표 1점 역지오코딩은
      부정확 — SAR 프레임이 대각선 스와스라 사각형엔 빈 바다가 많고, 대표점이
      한쪽 끝에 찍히면 반대쪽 끝의 실제 한국 영토를 놓침. 예: `satellite_
      inventory_sido_korean_*.csv`가 9919/D440/E067/4C8A/D298/3191/4C7C 등을
      "일본"으로 단일 태그했지만 이 씬들은 실제 검증된 침수 검출에 쓰였음).
      한반도 실경계와 **0% 겹침 확인된 것만** 제외: `CDFD`(6/30)·`1CE4`(7/12,
      같은 궤도, 서해 원해·중국), `F598`·`F05D`(7/4, 대한해협~일본 규슈).
      제외 씬은 zip을 삭제해 RTC 큐에서 자동 스킵되게 함.
      **주의(7/22)**: X드라이브(NAS) `rsync --ignore-existing`은 로컬에서 지운
      파일도 원본엔 남아있어 재실행 때마다 이 4개를 다시 끌고 옴 — rsync
      돌릴 때마다 `downloads/sentinel1_grd/`에 이 4개(CDFD/1CE4/F598/F05D)가
      재유입됐는지 확인하고 있으면 삭제할 것.

- [x] **한반도 전체(북한 포함) GRD 일괄 수집** (7/19) — `Korea_Peninsula.geojson`
      bbox로 6/25~7/18 재검색, 신규 44개 다운로드 → 총 73씬. 용량 확보를 위해
      SLC 원본 103GB를 `D:\06_SAR_system_archive\sentinel1`로 이동하고 기존
      경로에 junction 연결(스크립트 무수정 동작). 완료 후 RTC 끝난 GRD zip
      **60개도 NAS(`X:\02_Analysis\20260708_Flood\Sentinel-1`) 크기 검증 후
      로컬 삭제** (F: 35→143GB 확보). 사용자가 NAS 업로드 후 로컬 삭제하는
      방식으로 계속 운영 중 — 다운로드 폴더 점검 시 항상 NAS와 대조할 것.
- [x] **북한 관련 잔여 8씬 우선순위 처리** (7/20) — 처음엔 궤도 겹침 비율
      순으로 정했다가, 사용자 요청으로 **날짜 최신순**으로 재조정:
      `0B91`(7/19)→`3194`(7/19)→`2B06`(7/18)→`9FFF`(7/16)→`5D47`(7/7)→
      `525F`(7/7)→`1571`(7/4)→`794A`(6/25, 중국접경이라 최후순위). 진행 중
      `794A`가 150분(정상 최대치 93FC의 69분의 2배)까지 늘어져 멈춘 것으로
      판단해 강제종료 후 재시도 목록 맨 뒤로 배치.
      **2B06의 같은 궤도(12일 반복) 짝 정정**: 이전에 "6668·427D·74FD·F040·
      4D62(7/6 전체)"라고 잘못 안내함 — 정밀 대조 결과 같은 스와스 위치는
      **`427D` 하나뿐** (나머지 4개는 7/6 패스의 다른 위치, 무관).
- [x] **baseline v3 재구축 완료** (7/21) — 컷오프 7/3 유지 + `--fallback-dates
      20260704,20260706,20260707`로 빈틈메우기(`build_baseline_composite_grd.py`
      옵션 신규). baseline 수체 2,575→6,308 km²(북한 전역 커버).
- [x] **8개 날짜 전체 재탐지 완료** (7/21) — `7/4·7/7·7/13·7/14·7/15·7/16·
      7/18·7/19`를 v3로 일관 재계산 + 남/북 분리. [FLOOD_TIMELINE_KR.md]
      (FLOOD_TIMELINE_KR.md) 갱신 완료. **결론: 남한 수치는 신뢰 가능(7/14
      154km² 최대), 북한 수치는 대부분 아티팩트**(홍수 전 7/4·7/7에도 북한
      30~37km² 검출된 게 증거). v3는 북한 정량 추정엔 부적합.
- [x] **북한 동일궤도 pre/post 쌍 비교 완료** (7/21) — `detect_flood_grd_v2.py`에
      `--baseline`/`--tag` 옵션 추가해 같은 궤도 pre/post 1:1 차분. 결과(북한
      보수적): 7/13↔7/1 **190**, 7/18↔7/6 **254**, 7/19↔6/25 **465** km².
      (7/6·6/25 모자이크가 삭제된 중국 프레임/궤도 불일치로 처음 실패 → 로컬에
      남은 올바른 궤도 프레임만으로 `_s1d`/`_kr` 모자이크 재생성해 해결.)
- [x] **북한 정량화 불가 결론 확정** (7/21) — v3와 동일궤도 값이 크게 다르고,
      7/13↔7/1 습윤도 진단에서 **7/1이 7/13보다 4.1배 넓게 젖음** = 7/1
      baseline이 이미 젖음. **SPN 북한 날씨로 교차검증**: baseline 후보
      6/25·7/1·7/6 전부 강수일 → **마른 baseline 부재**가 근본 원인.
      상세: [FLOOD_NORTH_KOREA_KR.md](../flood/FLOOD_NORTH_KOREA_KR.md) 5·6절.
- [x] **baseline 무관 단일시기 수체 지도** (7/21) — `build_water_per_date.py`
      (날짜별, 6/25~7/19 전 날짜)·`build_water_single_scene.py`(단일 씬 →
      `scene_water/`) 신규. 변화가 아닌 "상태"라 baseline 문제와 무관.
      낡은 VRT(삭제된 소스 참조) 자동 재생성 로직 포함.
- [ ] **(다음) 북한: 장마 이전 baseline 확보** — 관측 기간(6/25~) 내내 북한에
      강수가 있어 마른 baseline이 없음. 6월 초·중순 같은 궤도 관측을 추가
      수집하거나, Sentinel-2 광학·수위계로 교차검증해야 북한 정량화 가능.
- [ ] (선택) 7/16 강원 동부 97km² 검출(남한)의 교차검증 — Sentinel-2 광학 또는
      공식 피해현황(kmz)과 대조.
- [x] **7/20 패스(궤도 008632, CE47·0CEF·DD29·F314·74BD·93DD) RTC 완료** (7/21) —
      392D 포함 7프레임 전부 RTC 완료. `flood_water_total_20260720.tif`는
      아직 미생성 (필요 시 `build_water_per_date.py 20260720`).
- [x] **신규 촬영 씬 재확인** (7/21) — 갱신된 `Korea.geojson`으로 CDSE STAC
      재조회, 7/20 21:32 UTC(74BD) 이후 신규 게시 없음 확인. 93DD가 갱신된
      AOI와 더 이상 안 겹치는 점 발견(경계가 좁아짐, 이미 받은 93DD 자체는
      영향 없음) — 다음 신규 검색 때 서쪽 경계 프레임 누락 여부 주의.
- [ ] **X드라이브(NAS) zip 아카이브 병합** — `wsl rsync --ignore-existing`로
      X드라이브 GRD zip 75개 중 로컬에 없던 59개를 `sentinel1_grd/`로 병합
      진행 중(F: 여유공간 524.7GB, 문제없음). 완료 후 로컬 인벤토리 재확인.
- [x] **footprint 재감사·FLOOD_TIMELINE_KR.md 정정** (7/22) — NAS rsync로
      제외 씬 재유입을 계기로 전체 zip footprint 재검증, 7/8·7/10 발표
      수치가 100% 바다 아티팩트임을 확정하고 문서 정정. 상세:
      [SCENE_FOOTPRINT_REAUDIT_KR.md](../pipeline/SCENE_FOOTPRINT_REAUDIT_KR.md).
- [ ] **7/17행(`D298`·`3191`·`4C7C`, 97.38km²) 격리 재계산** — 현재 저장된
      `flood_water_relaxed_20260716.tif`는 다른 궤도와 섞인 합성본이라 이
      3씬만의 값인지 확인 불가. `detect_flood_grd_v2.py --baseline --tag`로
      이 3씬만 모자이크해 재계산 필요(같은 궤도 003704 pre-event 짝 확인부터).

## P1(구) — post-event 영상 (7/14 시점 기록)

- [x] **S1C 하강 패스(2026-07-13 21:39~21:40 UTC, KST 7/14 06:39경) 카탈로그
      게시 확인** — 홍수일(7/8) 이후 최초 post-event 영상. 촬영→게시까지 실측
      약 4시간 걸림(오전 10:13 KST 확인 때는 게시 전이라 놓쳤었음, 10:43 게시).
- [x] **post-event GRD 다운로드 완료** (7/14 14:05~14:08) — 3개 신규:
      `93FC`/`3C22`/`1A5A` (`sentinel1_grd/`에 있음, 이 중 `3C22`·`1A5A`가 홍수
      AOI와 직접 겹침).
- [x] **post-event GRD RTC 완료** (7/14 14:08~15:54) — `93FC`(북한, AOI 무관)
      69.4분, `3C22`(AOI 겹침) 25.8분, `1A5A`(AOI 겹침) 10.5분. 뒤 두 개가 훨씬
      빠른 이유: 인접 pre-event 씬 처리 때 이미 Copernicus 30m DEM 타일이
      캐시돼 있었기 때문(93FC는 처음 다루는 위도대라 DEM을 새로 받음).
- [x] 모자이크 생성 — **단, `build_rtc_mosaic.py`가 아니라 `gdalbuildvrt`를
      직접 실행**했습니다. 이 스크립트는 `downloads/rtc`(SLC)만 읽도록
      하드코딩돼 있어 GRD(`downloads/rtc_grd`)는 지원하지 않기 때문
      (P5 항목 참고 — `--dir` 옵션 추가하면 스크립트로도 가능해짐). 대신
      P3의 NoData 버그 수정(`-srcnodata 0 -vrtnodata 0`)은 이번 명령에 반영
      해뒀습니다. 결과: `downloads/rtc_grd/s1_rtc_db_mosaic_20260713.vrt`
      (93FC 포함 3개 전부). 3C22·1A5A만 다시 묶고 싶으면 같은 명령에서
      93FC만 빼고 재실행.
- [ ] **post-event SLC는 보류 중** (사용자가 "다음에 하겠습니다"로 명시적으로
      미룸). 재개 시 대응 씬 ID: `S1C_IW_SLC__1SDV_20260713T213913..._41E9`,
      `..._213938..._64C0`, `..._214004..._04E2`. `conda run -n s1_pipeline
      python main_s1_list.py` (기존 파일은 자동 스킵) →
      `conda run -n s1_snappy python batch_slc_rtc.py`.

## P1.5 — 탐색적 분석: 북한 동일궤도(범위 밖, 필요 시에만)

93FC(7/13, 북한) footprint가 12일 주기 반복궤도라 7/1 씬(0FEB 등)과 바로 전/후
비교가 가능하다는 걸 발견해서 만들어둔 산출물들. **현재 프로젝트 핵심 범위
(충청권 홍수)는 아니므로 급하지 않음.**

- [x] 7/1·7/13 동일궤도 3프레임씩 날짜 모자이크 + 2밴드 스택 VRT + dB 차분
      계산 완료 (`downloads/rtc_grd/s1_rtc_db_diff_0701_vs_0713.tif`) — 평균
      +0.52dB, -3dB 이상 어두워진 픽셀 4.4%.
- [x] 같은 영역 HAND 22타일 다운로드 완료 (`downloads/hand/hand_north_orbit.vrt`).
- [ ] **차분 + HAND 결합해서 4.4%가 진짜 수체 변화인지 그림자/노이즈인지
      가려내기** — `build_baseline_water.py`의 `(dB 임계값) AND (HAND 임계값)`
      로직을 이 두 산출물에 재사용하면 됨. 다만 이건 baseline 4개 날짜 합집합이
      아니라 pre/post 각 1개 날짜 비교라 신뢰도는 낮음(스펙클 노이즈 영향 큼).

## P2 — 신규 침수 탐지 (핵심 목표 — ✅ 구현됨, 개선 항목만 남음)

- [x] **탐지 구현 완료** — `detect_flood.py` 계획은 `detect_flood_grd.py`(v1)
      → **`detect_flood_grd_v2.py`(현재 표준)**로 실현됨. dB<-16 + 하락폭
      -3dB(보수적)/무하락폭(느슨) 2단계, `--dates` 날짜 선택, post 씬 경계
      윈도우 최적화. 날짜별 결과는 [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md).
- [ ] `260709_침수피해현황_v2.kmz`(공식 피해 현황)와 대조해 confusion matrix
      스크립트 작성 → 임계값(-16dB/-3dB) 튜닝 근거 확보.
- [ ] HAND 결합 옵션 — 현재 v2는 HAND 미사용(전국 확장 시 타일 부족).
      `hand_aoi.vrt`+`hand_north_orbit.vrt` 범위에서만이라도 `--hand` 옵션 추가
      하면 그림자 오탐 감소 기대.
- [ ] baseline 궤도 방향(ascending/descending) 혼합 이슈 — post와 같은
      relative orbit의 baseline만 비교하는 `--orbit` 옵션 고려
      (CODE_REVIEW_KR.md P3-2). 시간 변화 추적에는 필수적
      (동일궤도 쌍: 7/1↔7/13, 6/26↔7/8, 6/28↔7/10, 6/27↔7/16, 7/6↔7/18).

## P3 — 결과값 정확도에 영향 (코드 수정, 언제든 가능)

- [ ] `build_rtc_mosaic.py:62` — `gdalbuildvrt`에 `-srcnodata 0 -vrtnodata 0` 추가.
      현재 프레임 겹침 구간에서 위 프레임의 빈 영역(0)이 아래 프레임의 유효 dB를
      덮어써 baseline 커버리지가 실제보다 줄어드는 버그. 수정 후
      **기존 VRT·baseline 재생성 필요**. (7/14, GRD post-event 모자이크는
      `gdalbuildvrt`를 직접 실행하면서 이 옵션을 수동으로 넣었지만, 스크립트
      본체는 아직 안 고쳐서 SLC 쪽 기존 VRT 4개는 여전히 버그 있는 상태.)
- [ ] `download_hand.py:83-91` — HAND 타일 다운로드에 `.part` 임시파일 +
      Content-Length 검증 추가 (`stac/download_s1.py` 패턴 재사용). 현재는
      중간에 끊긴 손상 타일이 재실행 시 "이미 있음"으로 조용히 통과됨.
- [ ] `build_baseline_water.py:108` — `dst_crs=src.crs`가 "전부 EPSG:4326"이라는
      주석에만 의존. UTM 등 다른 CRS 입력이 섞이면 조용히 어긋나므로 assert 추가.

## P4 — 이식성 (다른 컴퓨터로 옮길 계획이 있다면 우선순위 높음)

- [ ] **(7/14 사고 기록)** `git filter-repo --invert-paths --path .env` 실행 중
      로컬 `.env`가 디스크에서도 삭제됨 — `.gitignore`에 있어도 "추적되던 파일"이
      히스토리 재작성으로 트리에서 사라지면 작업 트리 정리 과정에서 함께 지워짐.
      reflog도 filter-repo가 즉시 만료시켜 git으로 복구 불가했음. **앞으로 히스토리
      재작성(filter-repo, rebase, reset --hard 등) 전에는 `.env`류를 프로젝트 밖으로
      먼저 복사해둘 것.**
- [ ] `prepro_gpt.py:41`, `prepro_grd_gpt.py:36` — `SNAP_BIN = r"C:\Program
      Files\snap\bin"` 하드코딩. 다른 PC는 SNAP 설치 경로가 다를 수 있으므로
      `.env`의 `SNAP_BIN`으로 이동.
- [ ] 같은 파일들의 `-c 14G`(gpt 캐시), `-q 8`(스레드 수)도 `.env`의
      `GPT_CACHE`/`GPT_THREADS`로 이동 — 램 32GB 미만 PC에서는 `14G`가 과함.
- [ ] `config.py`의 `OutputConfig.out_dir`이 상대경로(`./downloads`)라 실행
      위치에 따라 결과 폴더가 바뀜. `Path(__file__).resolve().parent /
      "downloads"`로 통일.
- [ ] `environment.yml` / `environment_snappy.yml`에 버전 고정이 없음
      (`python=3.10`만 고정). `conda env export --no-builds`로 lock 파일을
      만들어두면 다른 컴퓨터에서 같은 환경 재현이 쉬움. SNAP 버전(예: 12.x)도
      README에 기록 권장.

## P5 — 구조 정리 (급하지 않음, 유지보수성)

- [ ] `main_s1_list.py` / `main_s1_list_grd.py` 통합 (`--product slc|grd` 인자).
      겸사겸사 "검색 AOI와 교차하는 후보는 위성/날짜와 무관하게 전부 포함" 옵션
      추가 — 현재 top-k 로직은 같은 패스의 일부 프레임이 우연히 탈락할 수 있음
      (실례: 7/14 재검증 중 발견한 `S1A..._BA53` 미다운로드, 다행히 AOI상 중복이라
      손실은 없었음).
- [ ] AOI/geojson 처리 함수 4곳 중복(`main_s1_list*.py`, `prepro_gpt.py`,
      `build_baseline_water.py`, `download_hand.py`) → `geoutil.py`로 통합.
      `AOI_MARGIN_DEG = 0.1`도 세 곳에 흩어져 있음 → 한 곳으로.
- [ ] `batch_slc_rtc.py` / `batch_grd_rtc.py` 러너 중복 → 공용 함수로 추출.
      실패 시 "AOI 미교차(정상)"와 "그 외 실패"를 로그에서 구분 (현재는 SNAP 로그를
      직접 열어봐야 구분 가능).
- [ ] `stac/check_kml_dam_korea.py`, `stac/batch_check_kml_dams.py` — 이 저장소와
      무관한 K-water 댐 스크립트(하드코딩된 `E:\` 경로 포함). 별도 저장소로 이동.
- [ ] 정리 대상 파일: `downloads/rtc_grd`의 DEM 비교 실험 산출물(`*_dem_diff.tif`
      등 4개) → `experiments/` 하위로, 2022 Jeddah SLC(11GB) → 별도 보관,
      `output_partitioned_stac/`(출처 불명, 524KB) → 문서화 또는 삭제,
      `downloads/s1_frames_report_GRD.qmd` → 구버전 잔재, 삭제 권장
      (`s1_frames_report_GRD.geojson`은 7/14에 재생성해서 최신 상태 — 삭제 대상
      아님, 다만 export_frames_geojson.py에는 아직 GRD 전용 옵션이 없어 별도
      스크립트로 만듦. `export_frames_geojson.py`에 `--product grd|slc|all`
      옵션을 추가하면 이 별도 스크립트가 필요 없어짐).
- [ ] 최소 단위 테스트 추가: `stac/models.py`의 `parse_target_datetime_utc`,
      `make_datetime_range`; `stac/search_s1.py`의 `extract_product_id`;
      `prepro_gpt.py`의 `_iter_lonlat`/`aoi_wkt_from_geojson`.
- [ ] `print` 기반 로깅을 `logging` + 파일 핸들러(`downloads/logs/`)로 전환.

## 완료된 것 (참고용, 재작업 불필요)

- [x] GRD 검색·다운로드·RTC 파이프라인 전체 (14/14, 14/14)
- [x] SLC 검색·다운로드 전체 (14/14, EBE9 포함)
- [x] SLC RTC — 홍수 AOI 교차 씬 6/6 (나머지 8개는 AOI 밖이라 처리 대상 아님, 버그 아님)
- [x] HAND 다운로드, NGII DEM 클립, pre-event baseline 수체 지도
- [x] `.env` git 히스토리 세척 + force push (단, 비밀번호/키 자체 변경은 P0 참고)
- [x] git upstream 추적 연결(`git branch --set-upstream-to=origin/main main`)
- [x] post-event GRD 다운로드 + RTC + 모자이크 (3/3, 7/14 완료)
- [x] `s1_frames_report_GRD.geojson` 재생성 (7/14, 17개 프레임 반영)
- [x] GRD pre-event 날짜 모자이크 5종 + GRD baseline (`build_baseline_water_grd.py`,
      홍수 AOI, dB+HAND 합집합 — 7/14)
- [x] 남한 전역 최신관측 baseline (`build_baseline_latest_grd.py`, dB만·HAND 미사용·
      최근 영상 우선 — 7/14). detect_flood에서 post GRD와 비교할 기준 완비.
- [x] speckle 필터 비교(SNAP 4종 vs `filtering/` 자체구현) + `qa/` 4축 정량 평가
      패키지 커밋 (7/15, [FILTER_COMPARISON_KR.md](../pipeline/FILTER_COMPARISON_KR.md))
- [x] 재현 가능한 baseline 빌더 `build_baseline_composite_grd.py` (7/15)
- [x] 신규침수 탐지 v1/v2 + 전범위 확장 + 핫스팟/남북 분리 도구 (7/15~16,
      [FLOOD_DETECTION_KR.md](../flood/FLOOD_DETECTION_KR.md))
- [x] 한반도 전체 GRD 44씬 추가 수집 + SLC 원본 D: 이동(junction) (7/19)
- [x] baseline v2 재구축(pre-event 신규 17프레임 반영) + 7/8~7/16 날짜별
      침수 시간선 분석 (7/20, [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md))

## P0.6 — 북한 프레임 재처리 (2026-08-25 실측, 다음 세션 착수)

**북한 자료를 사용하기로 확정**(2026-08-25). `korea_full_cop30.tif`(≤39.9N)로
구워져 북부가 결측인 산출물을 `korea_peninsula_cop30.tif`(≤43.3N)로 다시 굽는다.
배경은 [ISSUES #17](ISSUES_KR.md) · [#24](ISSUES_KR.md).

VH RTC 105개 전수에서 **유효화소 20% 미만이 14개**다. 14개 **전부** 프레임이
39.9N을 넘는다 — DEM 범위가 원인임이 분명하다.

| 관측일 | 씬 | 유효% | 프레임 위도 | DEM 초과 |
| --- | --- | ---: | --- | ---: |
| 2026-07-03 | `32AE` | **0.00** | 41.30~43.20N | 3.30° |
| 2026-07-20 | `CE47` | **0.00** | 41.34~43.24N | 3.34° |
| 2026-07-01 | `5C8D` | **0.00** | 40.92~42.83N | 2.93° |
| 2026-07-13 | `4265` | **0.00** | 40.92~42.83N | 2.93° |
| 2026-07-25 | `3804` | **0.00** | 40.92~42.83N | 2.93° |
| 2026-07-03 | `64DE` | **0.00** | 39.80~41.70N | 1.80° |
| 2026-07-20 | `0CEF` | 0.21 | 39.84~41.74N | 1.84° |
| 2026-07-01 | `54D9` | 12.54 | 39.42~41.33N | 1.43° |
| 2026-07-13 | `AEB7` | 12.54 | 39.42~41.33N | 1.43° |
| 2026-07-25 | `4303` | 12.55 | 39.42~41.33N | 1.43° |
| 2025-07-05 | `C942` | 13.88 | 37.86~40.05N | 0.15° |
| 2025-07-17 | `FF05` | 13.88 | 37.86~40.05N | 0.15° |
| 2025-07-29 | `AAB6` | 13.88 | 37.86~40.05N | 0.15° |
| 2026-07-31 | `F0AE` | 14.05 | 37.86~40.02N | 0.12° |

- **전부 7월분이다.** 8월분은 [ISSUES #17](ISSUES_KR.md)이 2026-08-18에 이미
  씬별로 재고 판단했다(100% 손실 8장 등) — 그 목록과 합쳐서 진행할 것.
- 초과분 0.12~0.15°인 4개(`C942`·`FF05`·`AAB6`·`F0AE`)는 **남한 궤도**라
  손실이 프레임 북쪽 끝뿐이다. #17의 교훈대로 **전부 재처리는 틀린 반사** —
  남한 분석만 쓸 거면 그대로 둬도 된다.

- [x] ✅ **배치에 유효화소 하한 검사 추가 완료** (2026-08-25) — 이번 14개가
      전부 "성공"으로 집계됐던 근본 원인이었다([ISSUES #24](ISSUES_KR.md)).
      `gpt` 정상 종료 후에도 산출물을 열어 유효화소를 재고, 하한(기본 **5%**)
      미달이면 **실패로 세고 파일을 지운다**(안 지우면 다음 실행이 건너뛴다).
      - 재는 함수: [s1/core/rtc_qc.py](../../s1/core/rtc_qc.py) — 압축률의
        자료형도 여기서 `rasterio`에게 물어본다([ISSUES #20](ISSUES_KR.md) 동시 해결)
      - 배치 적용: [batch_runner.py](../../s1/preprocess/batch_runner.py)
        `_check_output()` — `run_batch`를 쓰는 **네 러너에 한 번에** 걸린다
      - 하한 조절: `--min-valid 20` (%) · `--min-valid 0` 이면 끔
      - 점검 도구: `python -m s1.tools.audit.rtc_integrity [씬ID...] [--csv ...]`
      - 검증: 빈 파일·NaN 파일은 삭제, 유효 10%짜리 부분 손실은 보존,
        `D635` 실측이 기록(유효 70.9% · 압축 43.2%)과 일치
- [ ] 재처리 대상 확정 — 위 14개 + #17의 8월 목록에서 **북한 손실이 실제로
      큰 것만**. 씬당 80~100분.
      ⚠ **8월분은 그 뒤 peninsula DEM 으로 다시 구워졌을 수 있다.** #17 목록을
      그대로 쓰지 말고 `rtc_integrity` 로 **지금 상태를 재고** 정할 것.
- [x] ✅ **원본 zip 14/14 전부 로컬에 있다** (2026-08-25 확인, 0.85~1.19 GB).
      다운로드 선행 없이 바로 재처리할 수 있다. (`downloads/sentinel1_grd/`,
      E: 이관분 정션 포함 180개)
- [ ] 재처리 명령: `--dem downloads/dem_basin/korea_peninsula_cop30.tif`
      ⚠ 파일명이 같아 **덮어쓰기**다. 먼저 지워야 배치가 건너뛰지 않는다.
      (대상 목록은 `rtc_integrity --min-valid 20 --only-bad --ids` 가 배치의
      `--only` 에 그대로 붙일 수 있는 쉼표 목록으로 뽑아준다.)

> **⏸ 착수 조건 (2026-08-25 19:40 확인)** — 다른 세션이 **P0.1c(26년 7월 잔여
> 18씬) 배치를 2병렬로 돌리는 중**이다(`gpt -c 7G` 2개). 여기서 재처리 배치를
> 더 띄우면 4병렬이 되어 **RAM 32 GB를 넘긴다.** 그 배치가 끝난 뒤 시작할 것.
> 남은 씬은 `32ED`·`D298`·`3191`·`4C7C` 정도로 보였다.
>
> ⚠ **그 배치는 유효화소 검사가 들어가기 전에 시작됐다**(실행 중인 파이썬은 옛
> 코드를 물고 있다). 끝나면 **26년 7월 산출물을 `rtc_integrity` 로 전수 점검**할 것.
>
> **08-25 19:36 시점까지 나온 12장은 점검 완료 — 하한 미달 0장**
> (`F040`·`525F`·`3194`·`DF80`·`74FD`·`5D47`·`0B91`·`427D`·`2B06`·`9FFF`·
> `639F`·`1571`, 유효 **42.9~66.8%**, 압축 25.8~39.7%).
> CSV: `temp/rtc_integrity_202607_new.csv` · 12장 7.3분(--jobs 1).
>
> 📌 **여기서 배운 것 — 유효화소 42%가 정상이다.** 스와스가 기울어져 북쪽 정렬
> 사각형의 네 모서리가 구조적으로 무효라, 기울기가 큰 북부 프레임은 기하만으로
> 40%대가 나온다. "정상 60~100%"는 2026-08 남한 위주 표본의 값이었다.
> 그래서 재처리 대상을 고를 때 **40%대를 손상으로 보면 안 된다** — P0.6 표의
> 14개는 0~14%로 그보다 훨씬 낮다.
