...
- [ ] **`geojson/South_Korea.geojson` 재검증** — `download_south_korea_month.py`
      ...
      결과가 바뀌면 문서를 갱신해야 한다.
- [ ] **공통 관측영역 마스크** — 같은 상대궤도라도 그날 찍힌 프레임 수가 달라
      관측 범위가 다르다. **(대권역 × 상대궤도)를 단위로** 양쪽 다 유효화소인
      픽셀만 남겨 집계해야 한다([DROUGHT_KR.md](../drought/DROUGHT_KR.md) 3절).
      (2026-08-14: 여기 예시로 들었던 "o003280 ↔ 계열 A" 짝은 상대궤도가
      서로 달랐던 것으로 밝혀져 삭제 —
      [RELATIVE_ORBITS_KR.md](../pipeline/RELATIVE_ORBITS_KR.md) 참고)
- [ ] **내수면 마스크** — 현재 면적은 서해·남해를 포함한 "상태 면적"이라
      가뭄 지표로 직접 못 쓴다.
- [x] **`batch_slc_rtc.py`도 `run_batch`로 통합** (2026-08-14 완료).
      AOI 미교차를 예외로 잡던 것을 **처리 전 footprint 판정**으로 바꿨다 —
      `s1/core/aoi.py`의 `intersects()`가 원본 zip의 map-overlay.kml로 경계와
      겹치는지 먼저 보고, 안 겹치면 목록에서 뺀다. 예전에는 "대상이 아닌 씬"과
      "진짜 실패"가 같은 칸에 집계돼 배치 결과를 믿을 수 없었다. 이제 남는
      실패는 전부 진짜 실패다. footprint를 못 읽는 제품은 **처리 대상으로
      남긴다**(걸러내는 쪽으로 틀리면 멀쩡한 씬을 조용히 빠뜨린다).
      덤으로 `--year/--aoi/--min-overlap/--no-aoi-filter/--gpt-q/--gpt-c`를
      인자로 노출했다(예전엔 상수 하드코딩). 이로써 **배치 러너 4개가 모두
      공통 뼈대**를 쓴다.
