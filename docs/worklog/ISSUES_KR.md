...
| #16 | (등록 대기) | 씬 ID 끝 4자리(제품 해시)로 대조하면 같은 촬영을 다르게 셈 |
| #17 | (등록 대기) | external DEM이 남한 전용 범위라 북한이 통째로 빠진다 |
| #18 | (등록 대기) | STAC 요약(`S1ItemSummary`)에 footprint가 없어 남한 판정이 늘 0장 |
| #19 | (등록 대기) | POEORB 미공개 시 RESORB로 조용히 대체 — `continueOnFail=false`가 못 막는다 |
| #20 | (등록 대기) | 압축률 QC 기준의 계산식이 없어 재현이 안 된다 |
...
**영향** — RESORB 자체의 정확도가 나쁜 것은 아니지만, **아카이브에 궤도 출처가
섞였다.** 연도 간 비교에서 기하 정합을 논할 때 이걸 모르면 설명 안 되는 어긋남
...
2. **09월 초(관측 +20일) 이후 같은 씬 1장을 POEORB로 다시 구워 기하 차이를
   실측**하고, 그 결과로 8월분 전량 재처리 여부를 결정한다. **재기 전에
   "다시 다 구워야 한다"고 단정하지 않는다** — #17에서 배운 것과 같다.
...
이 값은 **파일을 지울지 말지를 정하는 판정 기준**이다. 식이 없으면 다음 사람이
같은 숫자를 못 만든다. 실제로 2026-08 산출물 30장을 재점검할 때 두 기준이
달라 한 번 멈췄다.

### 해야 할 것

- 압축률을 내는 **함수를 도구로 고정**하고(예: `s1/tools/audit/` 아래),
  문서에는 그 도구 이름과 임계값만 적는다. 손으로 계산한 수치를 문서에
  박아두지 않는다.
- 임계값은 **절대값보다 분포로** 둔다. 실측상 정상은 25~44%(위 식 기준)로
  퍼져 있고 깨진 파일은 5.1%다 — **어느 식으로 재든 한 자릿수는 확연히 낮다.**
  "한 자릿수면 불량"이 식에 덜 민감한 규칙이다.
- 비영값 비율을 같이 본다. 압축률 하나로 판정하지 않는다.
