# 작업 진행 현황 (2026-07-21 기준, 아래 최신 절 참고)
...
> ## ⚠️ 현재 국면이 바뀌었습니다 (2026-08-14)
>
> 이 문서 본문은 **7월 홍수 모니터링** 시점 기록입니다. 그 이후 과제가
> **연도 간 가뭄 비교(25년 ↔ 26년, 이후 22년까지)** 로 옮겨갔고, 산출물 기준도
> 바뀌었습니다.
>
> | 항목 | 7월 홍수 국면 | 현재 (8월) |
> | --- | --- | --- |
> | 편파 | VV | **VH** (GEE 수체탐지와 정합, VV와 약 6 dB 오프셋) |
> | DEM | SNAP 자동 캐시 COP30 | **external `korea_full_cop30.tif`** |
> | 산출 폴더 | `downloads/rtc_grd_frost/` | `downloads/rtc_grd_frost_vh/` |
> | 비교 단위 | 날짜별 모자이크 | **(대권역 × 상대궤도)** |
> | 이 저장소 범위 | RTC + 수체탐지 | **RTC까지** (수체탐지는 별도 프로젝트) |
>
> 최신 상태는 [WORKLOG_20260814_KR.md](WORKLOG_20260814_KR.md)(상대궤도 재정립,
...
| 침수 시간선 분석 | ✅ [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md) — 7/8 당일 검출, 7/14~15 최대 154.4km²(남한) |
| speckle 필터 QA | ✅ [FILTER_COMPARISON_KR.md](../pipeline/FILTER_COMPARISON_KR.md) + `filtering/`·`qa/` 패키지 |
...
1. **전체 파이프라인 코드 리뷰 갱신** — [CODE_REVIEW_KR.md](../review/CODE_REVIEW_KR.md) 전면
...
1. **SNAP 필터 4종 비교**(multilook-only/Refined Lee/Lee/Frost, 1A5A 씬) 및
   ...
   [FILTER_COMPARISON_KR.md](../pipeline/FILTER_COMPARISON_KR.md) (커밋 `8792291`).
...
2. **`flood_hotspots.py`·`split_flood_area_nk_sk.py` 신규** — 2km 격자 핫스팟
   ...
   [FLOOD_DETECTION_KR.md](../flood/FLOOD_DETECTION_KR.md) 작성.
...
4. **침수 시간선 완성**: 7/8 당일 저녁 1.64km² 최초 검출(같은 궤도 6/26 FAA4
   ...
   상세: [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md). **⚠️ 2026-07-22 정정**:
...
4. **SPN 북한 날씨 교차검증**: `SPN_오늘의_북한날씨_20260625_20260721.md`
   ...
   결론을 [FLOOD_NORTH_KOREA_KR.md](../flood/FLOOD_NORTH_KOREA_KR.md)에 정리.
...
1. **RTC 필요성 문서화 + GTC 비교 산출**: [RTC_VS_GTC_KR.md](../pipeline/RTC_VS_GTC_KR.md)
...
2. **NAS rsync로 China/Japan 씬 재유입 → footprint 재감사로 확대**: X드라이브
   ...
   상세: [SCENE_FOOTPRINT_REAUDIT_KR.md](../pipeline/SCENE_FOOTPRINT_REAUDIT_KR.md).
3. **FLOOD_TIMELINE_KR.md 핵심 정정**: 위 0% 씬 중 `FAA4`/`3167`(7/8)과
   ...
   **100% 한반도 밖(바다)**으로 확정, 두 값 모두 무효 처리. "7/8 당일 위성으로
   ...
   격리 재계산 전까지 ⚠️미확정으로 표시. [FLOOD_NORTH_KOREA_KR.md](../flood/FLOOD_NORTH_KOREA_KR.md)
   궤도 표도 같이 정정.
...
5. **RTC vs GTC 대조 처리**: `prepro_grd_gpt.py`에 `build_grd_gtc_graph`(TF 생략,
   ...
   [GTC_RTC_PROCESSING_LOG_KR.md](../pipeline/GTC_RTC_PROCESSING_LOG_KR.md), 개념 설명:
   [RTC_VS_GTC_KR.md](../pipeline/RTC_VS_GTC_KR.md).
...
3. **(최우선) RTC 배치 완료 → 마무리 사이클**: ① 배치 종료 확인 → ② 7/19
   ...
   재계산 → ⑤ [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md) 수치 갱신.
