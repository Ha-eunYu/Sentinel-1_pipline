...
경계 파일은 [geojson/Korea_Peninsula.geojson](../../Korea_Peninsula.geojson)
...
핵심 로직은 [footprint_aoi.py](../../s1/footprint/footprint_aoi.py) 한 곳에 모았고, 파이프라인의
각 지점은 이 모듈을 가져다 쓰는 얇은 어댑터만 둔다.

| 파일 | 역할 | footprint_aoi에서 쓰는 함수 |
|---|---|---|
| [footprint_aoi.py](../../s1/footprint/footprint_aoi.py) | **공용 판정 로직 (단일 출처)** | — |
| [stac/search_s1.py](../../s1/stac/search_s1.py) | 다운로드 파이프라인의 자동 제외 필터 (`touches_korea`) | `footprint_intersects` |
| [verify_scene_footprint.py](../../s1/tools/audit/verify_scene_footprint.py) | 처리된 래스터의 물 픽셀 사후 검증 | `load_exterior_rings`, `points_in_rings` |
| [export_frames_geojson.py](../../s1/footprint/export_frames_geojson.py) | 실제 footprint를 QGIS용 GeoJSON으로 내보내기 | (STAC footprint 직접 사용, bbox는 fallback) |
...
- [SCENE_FOOTPRINT_REAUDIT_KR.md](SCENE_FOOTPRINT_REAUDIT_KR.md) — bbox 오판이
  낸 사고의 상세 감사 기록
- [FLOOD_TIMELINE_KR.md](../flood/FLOOD_TIMELINE_KR.md) — 무효 확정이 반영된 침수 타임라인
- [FLOOD_NORTH_KOREA_KR.md](../flood/FLOOD_NORTH_KOREA_KR.md) — 동일 패턴(baseline 커버리지
  아티팩트)의 최초 규명
