...
### 6-1. 합격 기준 — 유효화소 (2026-08-25 확정)

**`gpt`가 정상 종료한 것은 합격 근거가 아니다.** 프레임이 DEM 범위 밖이면
Terrain-Flattening 결과가 전부 무효가 되는데 SNAP은 오류 없이 끝낸다. 그렇게
만들어진 빈 파일이 **"성공"으로 집계된 채 105장 중 14장** 있었다(ISSUES #24).

| 지표 | 정상 (실측) | 배치가 지우는 선 |
| --- | --- | --- |
| 유효화소 (`유한값 ∧ ≠0 ∧ ≠nodata`) | 42 ~ 100% | **< 5%** |
| 압축률 (`가로×세로×밴드×자료형바이트` 대비) | 25 ~ 44% | 참고 지표 |

> ⚠ **유효화소가 낮다고 손상은 아니다.** 스와스는 궤도 방위각만큼 기울어진
> 평행사변형인데 산출물은 북쪽 정렬 사각형이라 **네 모서리가 구조적으로 무효**다.
> 2026-08 남한 위주 표본이 60~100%여서 한때 그것을 하한처럼 적었으나, 2026-07
> 북부 프레임 12장 실측은 **42.9~66.8%** 이고 전부 정상이다(2026-08-25).
> 40% 미만일 때만 "기하인지 DEM 밖인지" 따로 본다.

- 배치가 씬마다 자동으로 재고, **하한 미달이면 실패로 세고 파일을 지운다.**
  지우는 것이 규격이다 — 남기면 다음 실행이 "이미 처리됨"으로 건너뛴다.
- 하한은 `--min-valid`(%)로 조절한다. **끄지 말 것**(`--min-valid 0`).
- **수치를 손으로 계산해 문서에 박지 않는다.** 자료형을 눈으로 가정했다가 기준이
  정확히 2배 부풀려진 사고가 있었다(ISSUES #20). 계산은 항상 도구로 한다:

  ```bash
  conda run -n s1_snappy python -m s1.tools.audit.rtc_integrity --dir downloads/rtc_grd_frost_vh
  ```

- 5%는 **"사실상 빈 파일"만 걸러내는 선**이다. DEM 밖으로 북쪽 일부만 날아간
  씬(유효 12~14%)은 자료가 실제로 들어 있으므로 지우지 않는다. 그런 부분 손실은
  배치 로그의 유효화소 값으로 드러나며, 재처리 여부는 **씬별로 재서** 정한다(#17).
