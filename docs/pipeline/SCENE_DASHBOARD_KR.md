...
```text
CDSE 08-18 10:52   ·   로컬 10:52:50            [✓항상 위] [새로고침]
CDSE 8일 12개 · 미수신 0  |  보유 174  |  대기 79 · 처리중 2 · 완료 93

■ CDSE 최근 촬영
  촬영(KST)     궤도        위치                한반도  상태
  08-14 06:31  rel134 하행  경북·충남·충북        85%   대기
  08-14 06:31  rel134 하행  강원·경기·강원도(북)   85%   완료
  08-12 18:39  rel25  상행  황해남도·평안북도·평안남도 21%  전처리중

■ 이 PC 다운로드
  받은 때        씬                    크기      상태
  12시간 18분 전 S1C 08-08 18:22 B5F3  0.46 GB  대기

■ 전처리
  상태   씬                    비고
  처리중 S1C 08-07 06:39 0868  1시간 55분 전 시작 · 2.46 GB 기록
  대기   S1C 08-14 06:31 D635  1.12 GB
  완료   S1D 08-12 18:40 7AE5  1.97 GB · 1시간 2분 전
```

색: 빨강=손 볼 것(미수신·중단?), 파랑=진행 중(받는중·전처리중), 회색=끝난 것.
표는 보이는 줄보다 많이 담고 스크롤바가 붙는다. 창 제목에도 요약이 들어가
(`S1 현황 · 처리중 2 · 대기 79`) 최소화해 둬도 작업표시줄에서 읽힌다.
...
# 조회 일수·주기·창 크기 지정
powershell -File scripts\scene_dashboard.ps1 -Days 7 -CdseMinutes 30 -Geometry "600x780+40+40"
...
| `--geometry` / `-Geometry` | `600x760` | 창 크기(+위치) |
| `--cdse-rows` / `--local-rows` | 6 / 5 | 표에 보이는 줄 수(더 있으면 스크롤) |
...
2. **`--days` 창 밖의 촬영은 CDSE 표에 없다.** 로컬 표(다운로드·전처리)는
   창과 무관하게 보유분 전체를 센다.
