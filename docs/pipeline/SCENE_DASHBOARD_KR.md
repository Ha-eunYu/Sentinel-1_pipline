...
■ 전처리
  ...
  2026-07-31 18:40  S1D  CE22  대기   1.55 GB · 한반도 8%
  2026-07-17 06:16  S1D  4C7C  제외   0.93 GB · 한반도 0%
...
**제외도 목록으로 본다.** 전처리 표에 `제외` 줄이 회색으로 붙고, 비고 칸에
겹침%가 함께 찍힌다(대기 줄도 마찬가지다). 개수만 세면 "무엇이 왜 빠졌나"를
확인할 방법이 없어, 잘못 걸러진 씬이 조용히 묻힌다 — 겹침 판정이 이상하면
그 줄의 숫자가 바로 보인다. 목록은 `--proc-rows` 줄까지 **보이고**, 그 5배까지 담아 두어 나머지는 스크롤로
확인한다(전체 개수는 요약줄). 8줄만 담던 때는 19씬짜리 배치를 걸어도 절반이
화면에서 사라졌다(2026-08-19).
...
배치가 중간에 죽으면 임시폴더가 남는다. 그 씬을 나중에 다시 구우면 **산출물은
멀쩡한데 폴더만 남아**, 위 규칙대로면 영원히 '처리중'이 된다. 2026-08-25에
`525F`·`1571`이 그렇게 **엿새째 처리중**으로 보였다.

그래서 임시폴더를 셋으로 가른다.

| 판정 | 조건 |
| --- | --- |
| **잔여 임시폴더** | 산출물 mtime이 폴더 생성보다 뒤 + `--idle-minutes`(10분) 넘게 안 쓰임 → 그 씬은 끝났다. 처리중으로 세지 않고 "지워도 된다"고 알린다 |
| **중단?** | `gpt.exe`가 없고 `--stale-minutes`(30분) 초과, **또는** gpt가 돌아도 `--stale-hours`(6시간) 초과 |
| **처리중** | 나머지 |

`--idle-minutes` 조건이 핵심이다. gpt는 **굽는 내내 tif를 쓰므로**, "산출물이
있다"만 보면 지금 굽는 씬까지 잔여로 넘어간다(실제로 한 번 그렇게 됐다).
쓰기가 멈춘 지 10분이 지나야 끝난 것으로 본다.

gpt가 돌고 있어도 6시간을 넘기면 중단으로 보는 이유: 배치 두 개를 병렬로 돌리면
**다른 배치의 gpt를 보고** 죽은 폴더를 살아 있다고 오해한다.
...
갱신이 멈추면 상태줄이 `폴더 스캔 10:05:28  ⚠ 3시간째 멈춤`처럼 **멈췄다고**
말한다. 시각만 낡은 채로 두면 "잠깐 늦나 보다"와 "몇 시간째 죽어 있다"가
똑같아 보이기 때문이다.

- 예외는 `temp/logs/scene_dashboard.log`에 traceback 째로 쌓인다.
  `pythonw`는 stderr를 버리므로 이 파일이 유일한 단서다.
- **그리기 중 예외가 나도 갱신은 계속된다.** 2026-08-26에 `draw()`에서 예외가
  나면서 다음 tick 예약이 건너뛰어져 **세 시간 넘게 화면이 굳어 있었다.**
  제목줄(예외 직전에 쓴 값)과 요약줄(그 전 값)이 어긋난 것이 유일한 흔적이었다.
  지금은 `finally`에서 반드시 다시 예약하고, 오류는 상태줄과 로그로 보인다.
- 배치가 쓰고 있는 tif를 `stat()`하다 사라지는 경합도 그 원인 중 하나였다 —
  그 한 줄은 따로 감쌌다.
