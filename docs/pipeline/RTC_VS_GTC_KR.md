...
이 프로젝트의 GRD 전처리 그래프([prepro_grd_gpt.py](../../s1/preprocess/prepro_grd_gpt.py))가
...
2. **탐지 로직이 전 영상에 걸친 고정 임계값(-16 dB)** 하나로 물/비물을
   가릅니다 ([FLOOD_DETECTION_KR.md](../flood/FLOOD_DETECTION_KR.md)). 지형 밝기 왜곡이
   남아 있으면:
   ...
   단일시기 지도([FLOOD_NORTH_KOREA_KR.md](../flood/FLOOD_NORTH_KOREA_KR.md) 5·7절)까지
...
전체 씬 배치([batch_grd_rtc.py](../../s1/tools/preprocess/batch_grd_rtc.py)와 동일한 구조):
...
두 그래프는 [prepro_grd_gpt.py](../../s1/preprocess/prepro_grd_gpt.py)의 `build_grd_rtc_graph`
...
> 다릅니다. 이 프로젝트의 고정 임계값(-16 dB, [FLOOD_DETECTION_KR.md](../flood/FLOOD_DETECTION_KR.md))은
...
[build_water_per_date_otsu.py](../../s1/tools/water/build_water_per_date_otsu.py)(타일기반 Otsu
자동임계값, [OTSU_SPLIT_BASED_KR.md](../water/OTSU_SPLIT_BASED_KR.md))로 수체 지도를
...
`--fallback-db`(-16dB)로 물러난 경우([FLOOD_DETECTION_KR.md](../flood/FLOOD_DETECTION_KR.md)
