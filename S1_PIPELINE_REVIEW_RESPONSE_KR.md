...
- 감사 도구를 재현 가능하게 저장소에 추가:
  [footprint/audit_rtc_bbox_vs_footprint.py](footprint/audit_rtc_bbox_vs_footprint.py)
...
사이드카도 없습니다. [prepro_grd_gpt.py:212-220](prepro_grd_gpt.py#L212-L220)의
...
이미 버려집니다.** [stac/search_s1.py:99-113](stac/search_s1.py#L99-L113)의
...
[footprint/export_frames_geojson.py](footprint/export_frames_geojson.py) 가
...
파이프라인의 차분 구현 —
[detect_flood_grd_v2.py:214-217](detect_flood_grd_v2.py#L214-L217):
...
입니다. [detect_flood_grd.py](detect_flood_grd.py)(v1)는 처음부터 3중조건
보수판만 냅니다.
...
2. **파이프라인 문서가 이 산출물의 성격을 명시해 뒀습니다.**
   [FLOOD_NORTH_KOREA_KR.md](FLOOD_NORTH_KOREA_KR.md) 7절 "참고 — baseline 무관
...
- **v3 합성 baseline**: [build_baseline_composite_grd.py](build_baseline_composite_grd.py)
...
[FLOOD_NORTH_KOREA_KR.md](FLOOD_NORTH_KOREA_KR.md) 0·6절이 결론을 명시해
...
- 로컬 SAR 차분 경로에 JRC Global Surface Water 사용은 없습니다. 대신
  [build_baseline_water_grd.py:149](build_baseline_water_grd.py#L149)가
...
| 확인 항목 | 결과 |
|---|---|
| `exclude_non_korea`가 기본 동작인가 | ✅ **기본 True** — [stac/search_s1.py:151](stac/search_s1.py#L151) |
| 언제부터 적용됐나 | 커밋 `429d717`, **2026-07-23 12:51 KST**. 이후 `e687730`(7/27) footprint 모듈 통합 → `0baa1cf`(7/28) `footprint/` 폴더 정리 |
| 경계 파일이 `Korea_Peninsula.geojson`인가 | ✅ [footprint/footprint_aoi.py:46](footprint/footprint_aoi.py#L46). 제주 포함도 실측 확인 — `93DD` footprint 겹침을 독립 계산하니 **5.27%**, 문서에 기록된 값과 정확히 일치 |
| 느슨한 bbox 검색 + 사후 정밀 판정 구조인가 | ✅ [main_s1_list_grd.py:117-121](main_s1_list_grd.py#L117-L121) — `KOREA_SEARCH_BBOX = [123.0, 32.5, 131.5, 43.5]`, `intersects_geojson=None`. 정밀 판정은 검색 후 `touches_korea` |
| 필터 이전 산출물이 남아 인용되는가 | ⚠️ **아래 참조** |

### 재검증은 이미 끝나 있습니다

[SCENE_FOOTPRINT_REAUDIT_KR.md](SCENE_FOOTPRINT_REAUDIT_KR.md)가 **필터 도입
...
[build_water_per_date_otsu.py](build_water_per_date_otsu.py)가
...
([detect_flood_grd_v2.py:80](detect_flood_grd_v2.py#L80),
[build_baseline_water_grd.py:36](build_baseline_water_grd.py#L36),
[footprint/verify_scene_footprint.py:47](footprint/verify_scene_footprint.py#L47)),
...
- **픽셀 단위 교집합은 구현돼 있습니다** —
  [detect_flood_grd_v2.py:211](detect_flood_grd_v2.py#L211):
...
- **지역별 관측률 산출·속성 내보내기·저관측 경고는 없습니다.**
  [split_flood_area_nk_sk.py](split_flood_area_nk_sk.py)는 위도 38.3°로 남/북만
...
- Speckle-Filter: **Frost** (2026-07-23 Refined Lee → Frost 변경, 근거는
  ...
  기본값(Frost 3×3, damping 2) — [prepro_grd_gpt.py:83,93-95](prepro_grd_gpt.py#L83)에
  문서화돼 있습니다.
- DEM: **Copernicus 30m Global DEM** 기본, External DEM(NGII, `externalDEMApplyEGM=true`)
  옵션. 출력 화소 간격 10 m — [prepro_grd_gpt.py:48-67,194](prepro_grd_gpt.py#L48-L67).
...
- 수체 마스크 생성부에 MMU도, 형태학 연산도 없습니다.
  [build_water_per_date_otsu.py:215-240](build_water_per_date_otsu.py#L215-L240)의
  `write_water()`는 `db < threshold` 픽셀 임계값만 적용합니다.
- [water_area_report.py](water_area_report.py)의 `--min-area-m2`는 **면적 보고·
  GeoJSON 내보내기 단계 전용**이고 기본값 `0.0`입니다. 마스크 자체는 안 고칩니다.
- **GEE 경로엔 있습니다**: `connectedPixelCount(25).gte(min_connected)`,
  기본 `min_connected=8` ([gee/geeflood/config.py:26](gee/geeflood/config.py#L26)).
  로컬에 이식할 참조 구현이 됩니다.
...
- 있는 것: [qa/](qa/) 모듈(스페클 필터 QA — ENL/지표 비교),
  ...
  [footprint/verify_scene_footprint.py](footprint/verify_scene_footprint.py)
...
- 관련 문서 링크가 `README.md`·`TODO.md`로 적혀 있는데, 실제 파일명은
  [README_KR.md](README_KR.md)·[TODO_KR.md](TODO_KR.md)입니다.
