...
----------
    ...
    refined_lee_snap.py  Refined Lee (SNAP RefinedLee.java 충실 재현, 7x7 8방향)
...
- refined_lee: 경계 보존이 좋음(홍수 물/육지 경계 등). 계산량 약 5배.
  (단순화 구현 — SNAP보다 세게 평활화. FILTER_COMPARISON_KR.md 3-B 참고)
- refined_lee_snap: SNAP RefinedLee.java 충실 재현(7x7 8방향, sigmaV 국소추정,
  ENL 미사용). SNAP 없이 SNAP과 같은 Refined Lee가 필요할 때 사용.
...
from . import refined_lee_snap as _refined_lee_snap
from .base import (
    FilterFn,
    apply_lee_weight,
    integral_image,
...
from .refined_lee_snap import refined_lee_snap_filter
...
class FilterMethod(str, Enum):
    ...
    REFINED_LEE_SNAP = "refined_lee_snap"
    GAMMA_MAP = "gamma_map"
    FROST = "frost"
    LEE_SIGMA = "lee_sigma"
    MEDIAN = "median"
...
_MIN_WINDOW = {
    ...
    FilterMethod.REFINED_LEE_SNAP: _refined_lee_snap.MIN_WINDOW_SIZE,
...
def make_filter_fn(
    ...
    if method is FilterMethod.REFINED_LEE_SNAP:
        return _refined_lee_snap._snap_refined_lee_fn(col_block)
