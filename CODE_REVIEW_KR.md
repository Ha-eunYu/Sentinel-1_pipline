...
- **총평**: 검색→다운로드→RTC→모자이크→수체 baseline까지 이어지는 흐름이 명확하고,
  ...
  다만 **git에 커밋된 자격증명 1건(긴급)**, 데이터 정합성에 영향을 주는 버그 3건,
  그리고 대규모 코드 중복이 남아 있음.
...
| 등급 | 건수 | 내용 |
|------|------|------|
| **P0 (긴급/보안)** | 1 | `.env`(CDSE 계정/비밀번호)가 git에 커밋되어 있음 |
| **P1 (결과값에 영향)** | 4 | 모자이크 NoData 미지정, HAND 다운로드 무결성, baseline CRS 가정, GRD 산출물 미사용 |
| **P2 (견고성/중복)** | 8 | 다운로드 재시도 로직, 코드 중복(4계열), 하드코딩, 실험 산출물 정리 등 |
| **P3 (다음 단계/품질)** | 5 | detect_flood 미구현, 임계값 검증, 궤도 방향 혼합, 테스트/로깅 부재 등 |

---

## P0 — 보안: `.env`가 git에 추적됨 (즉시 조치)

`.gitignore`에 `.env`가 있지만 **이미 추적 중인 파일에는 gitignore가 적용되지 않습니다.**
확인 결과 `.env`(CDSE_USERNAME / CDSE_PASSWORD 실값 포함)가 커밋 `b41b9b6`(STAC_serch)부터
git 이력에 들어가 있습니다.

**조치 순서:**

```bash
# 1) 추적 해제 (파일 자체는 로컬에 유지)
git rm --cached .env
git commit -m "Remove .env from tracking"

# 2) CDSE 비밀번호 즉시 변경 (이력에 이미 남았으므로 필수)
#    https://dataspace.copernicus.eu 계정 설정

# 3) 원격 저장소에 push한 적이 있다면 이력 자체를 세척
#    (로컬 전용 저장소라면 2번까지로 충분하다고 판단해도 됨)
git filter-repo --invert-paths --path .env
```

참고: `.gitignore`의 `.env.*` 패턴은 `!.env.example`로 예외 처리되어 있어 example은 안전.
...
### 2. `download_hand.py` — 다운로드 무결성 검증 없음 ([download_hand.py:83-92](download_hand.py#L83-L92))
...
### 4. GRD RTC 산출물(≈14GB 처리 결과)이 다운스트림에서 미사용

`build_rtc_mosaic.py`는 `downloads/rtc`(SLC)만 읽고, `build_baseline_water.py`도 SLC
모자이크만 씁니다. `batch_grd_rtc.py`로 만든 `downloads/rtc_grd/*.tif` 16개는 QGIS 수동
확인 외에 파이프라인 어디에도 연결되어 있지 않습니다. 의도(광역 육안 확인용)라면 README에
명시하고, 아니라면:
...
### 5. `main_s1_list.py` ↔ `main_s1_list_grd.py` — 사실상 전체 중복 (각 ~215줄)
...
`build_baseline_water.aoi_bbox`는 `coordinates[0]`만 읽으므로 MultiPolygon이나
FeatureCollection(피처 2개 이상)이 오면 **조용히 첫 링만 반영**됩니다. `prepro_gpt`의
재귀 방식이 가장 견고하므로, `geoutil.py` 하나로 통합 후 전부 그것을 쓰세요:
...
### 7. `batch_slc_rtc.py` ↔ `batch_grd_rtc.py` — 러너 구조 중복
...
- `YEAR_FILTER = "_2026"` 같은 부분 문자열 매칭은 씬 ID 다른 위치와 우연히 일치할 수
  있으므로 `DATE_PATTERN`(build_rtc_mosaic의 정규식)을 재사용해 날짜를 파싱해서 비교 권장.

### 8. `stac/download_s1.py` — 재시도 로직 다듬기
...
### 10. `prepro_gpt.py` / `prepro_grd_gpt.py` — 하드코딩
...
### 11. `compare_dem_rtc.py` — 차분 맵 비압축 저장
...
### 12. `stac/check_kml_dam_korea.py`, `stac/batch_check_kml_dams.py` — 이 저장소 소속이 아님
...
- `classify_one_kml()`의 행 구성 루프는 `check_kml_dams()`와 중복.
- geopandas는 `environment.yml` 어디에도 없어 현재 두 환경 모두에서 실행 불가.
...
| 폴더 | 내용 | 상태 |
|------|------|------|
| `sentinel1/` (SLC 원본) | 2026년 씬 12개 (6/25~7/3) + 2022 Jeddah 1개 | ⚠ 미완 1개 |
| `sentinel1_grd/` (GRD 원본) | 14개 (6/25~7/3) | 정상 |
| `rtc/` (SLC RTC) | 6개 + 날짜별 VRT 4개 (6/25, 6/26, 7/1, 7/2) | 정상 |
| `rtc_grd/` (GRD RTC) | 16개 + DEM 비교 실험 6개 | ⚠ 실험 잔재 |
| `hand/` | 타일 4개 + VRT | 정상 |
...
1. **미완 다운로드 1건**: `S1C_..._20260626T213052_..._EBE9.zip.part` (3.3GB).
   `main_s1_list.py` 재실행으로 이어받기 가능. 6/26 baseline 커버리지가 이 프레임만큼
   빠져 있을 수 있으므로, 완료 후 `batch_slc_rtc.py` → `build_rtc_mosaic.py 20260626` →
   `build_baseline_water.py` 재실행 필요.
2. **7/3 SLC 3개(약 23GB)는 다운로드됐지만 RTC 산출물이 없음** — 21:23 패스가 홍수 AOI와
   미교차라 batch에서 스킵된 것으로 보이나(GRD 7/3은 전체 씬 처리라 산출물 있음),
   `PRE_EVENT_DATES`에도 7/3이 없어 어느 쪽이 의도인지 코드만으로는 알 수 없습니다.
   batch 러너가 "AOI 미교차 스킵" 목록을 파일로 남기면(단순 txt) 이런 확인이 쉬워집니다.
3. **`PRE_EVENT_DATES` 하드코딩** ([build_baseline_water.py:66](build_baseline_water.py#L66)):
   `rtc/s1_rtc_db_mosaic_*.vrt` 글롭으로 자동 발견하되 홍수일(7/8) 이전만 포함하는 방식으로
   바꾸면 새 pre-event 씬 추가 시 코드 수정이 필요 없어집니다. 홍수일을 상수
   `EVENT_DATE = "20260708"`로 두고 필터.
4. **2022 Jeddah SLC(11GB)**: 현재 캠페인과 무관한 과거 참조용. `YEAR_FILTER` 덕에 처리는
   안 되지만 디스크만 차지 — 별도 보관 폴더로 이동 권장.
5. **DEM 비교 실험 잔재**: `*_dem_diff.tif`(673MB) / `*_dc_dem_diff.tif` / `*_rtc_db_dc_cop.tif`
   / `*_dc_ngii.tif` 등 접두사·접미사 규칙이 섞인 산출물이 rtc_grd에 혼재. 실험 결과는
   `rtc_grd/experiments/` 하위로 분리하고 명명 규칙(항상 접미사) 통일 권장 —
   현재 상태로는 `*_rtc_db*.tif` 글롭을 쓰는 후속 코드가 실험 파일을 오인 수집할 수 있음
   (실제로 `build_grd_rtc_graph`의 out_tag 방식과 compare_dem_rtc의 tag 방식이 다름).
6. **`output_partitioned_stac/output_partitioned_stac.zip`(524KB)**: 어떤 스크립트도 이
   폴더를 참조하지 않고 README에도 없음. 출처 불명 산출물 — 필요하면 문서화, 아니면 삭제.
7. **`downloads/s1_frames_report_GRD.geojson` + `.qmd`**: 현재 `export_frames_geojson.py`는
...
### 1. detect_flood.py (핵심 목표) — 아직 미구현

홍수일이 7/8인데 확보된 씬이 7/3까지이므로 post-event 영상이 아직 없습니다. 구현 시 권장 구조:
...
| 순서 | 작업 | 규모 |
|------|------|------|
| 1 | `.env` git 추적 해제 + CDSE 비밀번호 변경 (P0) | 즉시 |
| 2 | `build_rtc_mosaic.py`에 `-srcnodata 0 -vrtnodata 0` 추가 후 VRT·baseline 재생성 (P1-1) | 10분 + 재실행 |
| 3 | `download_hand.py` .part 원자적 다운로드 (P1-2) | 30분 |
| 4 | 미완 SLC(.part) 이어받기 → 6/26 재처리 (결과 1) | 재실행 |
| 5 | `geoutil.py` 통합 + `AOI_MARGIN_DEG` 단일화 (P2-6) | 반나절 |
| 6 | main_s1_list 통합(SLC/GRD), argparse화 (P2-5) | 반나절 |
| 7 | detect_flood.py 설계·구현 (P3-1) — post 영상 확보 대기 중 골격 먼저 | 1~2일 |
| 8 | 다운로드 재시도 개선, batch 러너 공용화, 하드코딩 제거 (P2-7/8/10) | 1일 |
| 9 | 실험 산출물 정리 + 불명 파일 정리 (결과 5/6/7) | 1시간 |
| 10 | 최소 테스트 + 로깅 도입 (P3-4/5) | 1일 |
