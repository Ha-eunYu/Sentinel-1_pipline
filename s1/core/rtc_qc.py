...
# 참고용 정상 범위. 판정 기준이 아니라 보고서에 찍는 맥락이다.
#
# ⚠ **유효화소가 낮다고 손상은 아니다.** 스와스는 궤도 방위각만큼 기울어진
# 평행사변형인데 산출물은 북쪽 정렬 사각형이라, 네 모서리 삼각형이 구조적으로
# 무효다. 기울기가 클수록 그 몫이 커진다 — 2026-07 북부 프레임 12장 실측이
# **42.9~66.8%** 였고 전부 정상이다(2026-08-25). 60~100%는 2026-08 남한 위주
# 표본에서 나온 값이라 그대로 하한처럼 쓰면 멀쩡한 프레임을 의심하게 된다.
NORMAL_VALID = (0.60, 1.00)
# 이 아래면 기하만으로는 설명이 어렵다 — DEM 밖인지 따로 볼 값.
LOW_VALID = 0.40
